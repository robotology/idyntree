...
class ExtWrenchesAndJointTorquesEstimator
{
    ...
    operator=(const ExtWrenchesAndJointTorquesEstimator& /*other*/)
    {
        return *this;
    };
...
public:
     ...
     * @return true if all went well (model and sensors are well formed), false otherwise.
     */
    IDYNTREE_DEPRECATED_WITH_MSG("Deprecated, please use variant of this method (i.e. setModel) "
                                 "where SensorsList is passed via the iDynTree::Model.")
     ...
     *
     * @note this will create e a reduced model only with the joint specified in consideredDOFs and
     * the fixed joints in which FT sensor are mounted.
     *
     * @param[in] filename path to the file to load.
     ...
     */
    bool
    loadModelAndSensorsFromFileWithSpecifiedDOFs(const std::string filename,
     ...
     * @param[in] properClassicalLinearAcceleration proper (actual acceleration-gravity) classical
     * acceleration of the origin of the specified frame, expressed in the specified frame
     * orientation.
     * @param[in] angularVel angular velocity (wrt to an inertial frame) of the specified floating
     * frame, expressed in the specified frame orientation.
     * @param[in] angularAcc angular acceleration (wrt to an inertial frame) of the specified
     * floating frame , expressed in the specified frame orientation.
    ...
    /**
     * \brief For each submodel without any external wrench, computes the equation that relates the
     * FT sensor measures with the kinematics-related known terms.
     *
     ...
     * \f]
     * Where:
     *   * \f$w\f$ is the vector of dimension 6*nrOfFTSensors obtained by stacking the FT sensors
     * measures:
     *   * \f$A\f$ is a matrix of size 6 x 6*nrOfFTSensors that depends on position in space of the
     * FT sensors
     *   * \f$b\f$ is a vector of size 6 that depends on the position, velocity, acceleration and
     * gravity of each link in the submodel
     *
     * This function provides an easy way to compute A and B . Typically, these quantities are not
     * used online during the estimation of external wrenches or  internal torques, but rather as an
     * helper method when calibrating FT sensors.
     *
     * In the rest of the documentation, we will refer to this quantities:
     *   * nrOfSubModels (\f$n_{sm}\f$): the number of submodels in which the model is divided, as
     * induced by the FT sensors present in the model.
     *   * nrOfSubModelsWithoutExtWrenches: the number of submodels on which there is no external
     * wrench
     *   * nrOfFTSensors (\f$n_{ft}\f$): the number of FT sensors present in the model
     * In particular, the value of A and b for a given submodel is the following. First of all, for
     * any submodel $sm$ with no external force, we can write (from Equation 4.19 of
     * https://traversaro.github.io/traversaro-phd-thesis/traversaro-phd-thesis.pdf, modified to
     * account for all FT sensors in the submodel and to remove external wrenches):
     *
     * \f[
     *   \sum_{s=1}^{n_ft}~\mu_{sm, ft}~\sigma_{sm, ft}~{}_{B_{sm}} X^{ft} \mathrm{f}^{meas}_{ft} =
     * \sum_{L \in \mathbb{L}_{sm}} {}_{B_{sm}} X^{L} {}_L \phi_L
     * \f]
     *
     * where:
     *  * \f$ \mu_{sm, ft} \f$ is equal to \f$1\f$ if the sensor \f$ft\f$ is attached to the
     * submodel \f$sm\f$, and \f$0\f$ otherwise
     *  * \f$ \sigma_{sm, ft} \f$ is equal to \f$1\f$ if the sensor \f$ft\f$ is measuring the force
     * applied on submodel \f$sm\f$ or \f$-1\f$ if it is measuring the force that the submodel
     * excerts on its neighbor submodel
     *  *\f$ {B_{sm}} \f$ is a frame in which this equation is expressed, that for this function it
     * is the base link of the submodel.
     *
     *
     * With this definitions, we can see that A_sm and b_sm for a given submodel \f$sm\f$ can be
     * simply be defined as:
     *
     * \f$
     * A_{sm} = \begin{bmatrix} \mu_{sm, ft0}~\sigma_{sm, ft(0)}~{}_C X^{ft(0)}  & \hdots & \mu_{sm,
     * ft(n_ft-1)}~\sigma_{sm, ft(n_ft-1)}~X^{ft(n_ft-1)} \end{bmatrix}
     * \f$
     *
     * \f$
     * w_{sm} = \begin{bmatrix} \mathrm{f}^{meas}_{ft(0)}  \\ \vdots
     * \\ \mathrm{f}^{meas}_{ft(n_ft-1)}  \end{bmatrix}
     * \f$
     *
     * \f$
     ...
     *          updateKinematicsFromFixedBase must be called.
     *
     * @param[in] unknowns the unknown external wrenches, that is used to understand the submodels
     * in which no external wrench is present
     ...
     * @param[out] subModelIDs vector of size nrOfSubModelsWithoutExtWrenches of unsigned integers
     * from 0 to nrOfSubModels-1, subModelIDs[i] specifies to which submodel the quantities A[i]
     * @param[out] baseLinkIndeces vector of size nrOfSubModelsWithoutExtWrenches of
     * iDynTree::LinkIndex from 0 to nrOfLinks-1, baseLinkIndeces[i] specifies the link in which the
     * equation i is expressed
     * @return true if all went well, false otherwise.
     */
    bool computeSubModelMatrixRelatingFTSensorsMeasuresAndKinematics(
        const LinkUnknownWrenchContacts& unknowns,
    ...
    /**
     * \brief Estimate the external wrenches and the internal joint torques using the measurement of
     * the internal F/T sensors.
     *
     ...
     * Contact force estimations using tactile sensors and force/torque sensors.
     * URL :
     * http://www.researchgate.net/profile/Andrea_Del_Prete/publication/236152161_Contact_Force_Estimations_Using_Tactile_Sensors_and_Force__Torque_Sensors/links/00b495166e74a5369d000000.pdf
     *
     * \note There should be at least 6 unknowns variables for each submodel for which the
     * estimation of external wrenches is performed (i.e. usually 1 unknown wrench for each
     * submodel). If is not the case, undefined results could occur in the estimation.
     *
     * @param[in] unknowns the unknown external wrenches
     ...
     * @return true if the model is still, false if it is moving or if the kinematics was never
     * setted.
     *
     * \note This method can be computationally expensive, so in most case it
     *       may be a better idea to just do the check on the input variables (joint velocities,
     * joint acceleration).
     */
    ...
    /**
     * Compute the vector of the sum of all the wrenches (both internal and external, excluding
     * gravity) acting on link i, expressed (both orientation and point) with respect to the
     * reference frame of link i, using the articulated body model and the kinematics information
     * provided by the updateKinematics* methods.
     *
