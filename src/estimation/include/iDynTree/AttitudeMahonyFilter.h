...
/**
 * @class AttitudeMahonyFilter Implements an explicit passive complementary filter on quaternion
 * groups described in the paper <a
 * href="https://hal.archives-ouvertes.fr/hal-00488376/document">Non-linear complementary filters on
 * SO3 groups</a>
 *
 * The filter is used to estimate the states \f$ X = \begin{bmatrix} {^A}q_B \\ {^B}\Omega_{A,B}
 * \\ {^B}b \end{bmatrix}^T \f$ where \f$ {^A}q_B \in \mathbb{R}^4 \f$  is the quaternion
 * representing the orientation of a body(IMU) frame with respect to an inertial frame ,
 ...
 * convenience
 *
 * The discretized dynamics of the filter is implemented in the propagateStates() method and is
 * described by the following equations,
 * \f$ q_{k+1} = q_{k} + \Delta t \frac{1}{2}q_{k} \circ \begin{bmatrix} 0 \\ {\Omega_y}_{k+1} - b_k
 * + K_p \omega_{mes_{k+1}}\end{bmatrix}\f$
 * \f$ \Omega_{k+1} = {\Omega_y}_{k+1} - b_k \f$
 * \f$ b_{k+1} = b_k - K_i \Delta t \frac{1}{2} \omega_{mes_{k+1}} \f$
 *
 * The updateFilterWithMeasurements() uses the recent IMU measurements to compute the term \f$
 * \omega_{mes} \f$ which gives the vectorial from accelerometer and magnetometer measurements
 * \f$ \omega_{mes} = -(\Sigma{n}{i=1} \frac{k_i}{2} (v_i \hat{v}_i^T - \hat{v}_i v_i^T) )^{\vee}
 * \f$ where \f$ v_i \f$ is the normalized accelerometer or magnetometer measurement,
 * \f$ \hat{v_i} \f$ is the vector obtained from the orientation estimated combined with gravity
 * direction or absolute magnetic field direction, for e.g, \f$ \hat{v_acc} = {^w}R_b^T e_3 \f$ and
 * \f$ k_i \f$ is the confidence weight on the i-th measurement. In our case, i = 1 or 2.
 *
 * The usage of the attitude estimator can be as follows,
 * - After instantiation, the parameters of the filter can be set using the individual parameter
 * methods or the struct method.
 ...
 *     - propagateStates() method to propagate the states through the system dynamics and correcting
...
public:
    ...
    /**
     * @brief Set the gravity direction assumed by the filter (for computing orientation vectorial
     * from accelerometer)
...
private:
    ...
    iDynTree::Vector3 m_omega_mes; ///< vectorial estimate from accelerometer and magnetometer
                                   ///< measurements, \f$ \omega_{mes} \in \mathbb{R}^3 \f$,
                                   ///< notation from the paper Non linear complementary filters on
                                   ///< the special orthogonal group
    iDynTree::GyroscopeMeasurements m_Omega_y; ///< gyroscope measurement, \f$ \Omega_{y} \in
                                               ///< \mathbb{R}^3 \f$, notation from the paper Non
                                               ///< linear complementary filters on the special
                                               ///< orthogonal group
    ...
    iDynTree::Direction m_gravity_direction; ///< direction of the gravity vector expressed in the
                                             ...
                                             ///< \end{bmatrix}^T \f$
    iDynTree::Direction m_earth_magnetic_field_direction; ///< direction of absolute magnetic field
                                                          ///< expressed in the inertial frame
                                                          ///< denoted by \f$ A \f$, default set to
                                                          ///< \f$ {^A}m = \begin{bmatrix} 0 & 0
                                                          ///< & 1.0 \end{bmatrix}^T \f$
};
