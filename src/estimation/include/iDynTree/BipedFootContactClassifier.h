 ...
 * The switching pattern is primarily used to change the frames of reference for the Legged Odometry
 * (LO) while one of the two foot breaks contact with the ground surface, since LO assumes that
 * atleast one foot is in contact with ground at any instant of time. However, it is also used to
 * assign the primary foot in case of double stance.
 *
...
public:
    ...
    /**
     * Constructor
     * @params leftFootSchmittParams const ref to struct containing parameters for the left foot
     * schmitt trigger device
     * @params rightFootSchmittParams const ref to struct containing parameters for the right foot
     * schmitt trigger device
     */
    BipedFootContactClassifier(const SchmittParams& leftFootSchmittParams,
                               const SchmittParams& rightFootSchmittParams);
     ...
     */
    void updateFootContactState(double currentTime,
     ...
     */
    contactFoot getPrimaryFoot()
    {
        return m_primaryFoot;
    }
     ...
     */
    bool getLeftFootContactState()
    {
        return m_leftFootContactState;
    }
     ...
     * @return true if in contact, false otherwise
     */
    bool getRightFootContactState()
    {
        return m_rightFootContactState;
    }
     ...
     * In case it is set to UNKNOWN_FOOT, it waits for the foot normal force measurements (checks
     * left first and then right), and activates corresponding foot, handled in the
     * detectTransitions() method.
