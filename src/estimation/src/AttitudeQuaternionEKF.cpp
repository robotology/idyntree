...
void iDynTree::AttitudeQuaternionEKF::prepareSystemNoiseCovarianceMatrix(iDynTree::MatrixDynSize& Q)
{
    ...
    iDynTree::MatrixDynSize Fu_dyn(m_state_size,
                                   m_input_size + m_state_qekf.m_gyroscope_bias.size());
    Fu_dyn.zero();
...
void iDynTree::AttitudeQuaternionEKF::prepareMeasurementNoiseCovarianceMatrix(
...
bool iDynTree::AttitudeQuaternionEKF::initializeFilter()
{
    ...
    // ekfinit resizes and initializes the filter matrices and vectors to zero - dont move it frome
    // here
    if (!ekfInit())
    {
        return false;
...
bool iDynTree::AttitudeQuaternionEKF::propagateStates()
{
    ...
    {
        iDynTree::reportError("AttitudeQuaternionEKF",
    ...
    const iDynTree::MagnetometerMeasurements& magMeas)
{
    ...
    // compute yaw angle from magnetometer measurements by limiting the vertical influence of
    // magentometer to do this first rotate the measurement to inertial frame, set z-component to
    // zero. convert back to body frame and compute the yaw using atan2(y, x)
                                                          ...
                                                          iDynTree::MatrixDynSize& F)
{
    ...
    dfq_by_dq(0, 0) = dfq_by_dq(1, 1) = dfq_by_dq(2, 2) = dfq_by_dq(3, 3)
        = (2.0 / m_params_qekf.time_step_in_seconds);
    ...
    toEigen(dfgyrobias_by_dgyrobias)
        *= (1 - (m_params_qekf.bias_correlation_time_factor * m_params_qekf.time_step_in_seconds));
                                                          ...
                                                          iDynTree::MatrixDynSize& H)
{
    ...
    {
        reportError("AttitudeQuaternionEKF",
                    "computejacobianH",
...
bool iDynTree::AttitudeQuaternionEKF::ekf_f(const iDynTree::VectorDynSize& x_k,
...
bool iDynTree::AttitudeQuaternionEKF::ekf_h(const iDynTree::VectorDynSize& xhat_k_plus_one,
                                            iDynTree::VectorDynSize& zhat_k_plus_one)
{
...
bool iDynTree::AttitudeQuaternionEKF::getDefaultInternalInitialState(
...
bool iDynTree::AttitudeQuaternionEKF::getInternalState(
    const iDynTree::Span<double>& stateBuffer) const
{
...
bool iDynTree::AttitudeQuaternionEKF::setInternalStateInitialOrientation(
