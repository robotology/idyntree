...
void testBerdySensorMatrices(BerdyHelper& berdy, std::string filename)
{
    ...
    ForwardPosVelAccKinematics(berdy.model(),
                               berdy.dynamicTraversal(),
                               pos,
                               vel,
                               generalizedProperAccs,
                               linkPos,
                               linkVels,
                               linkProperAccs);
    RNEADynamicPhase(berdy.model(),
                     berdy.dynamicTraversal(),
                     pos.jointPos(),
                     linkVels,
                     linkProperAccs,
    ...
    extWrenches(berdy.dynamicTraversal().getBaseLink()->getIndex())
        = extWrenches(berdy.dynamicTraversal().getBaseLink()->getIndex()) + genTrqs.baseWrench();
    ...
    for (LinkIndex visitedLinkIndex = 0; visitedLinkIndex < berdy.model().getNrOfLinks();
         visitedLinkIndex++)
    {
    ...
    berdy.updateKinematicsFromFloatingBase(pos.jointPos(),
                                           vel.jointVel(),
                                           baseIdx,
                                           linkVels(baseIdx).getAngularVec3());
    ...
    VectorDynSize dynamicsResidual(berdy.getNrOfDynamicEquations()),
        zeroRes(berdy.getNrOfDynamicEquations());
    ...
    */
    ...
    {
        std::cout << "BerdyHelperUnitTest, testing sensors matrix for model " << filename
                  << std::endl;
        ...
        bool ok = predictSensorsMeasurementsFromRawBuffers(berdy.model(),
                                                           berdy.sensors(),
                                                           berdy.dynamicTraversal(),
                                                           linkVels,
                                                           linkProperAccs,
        ...
        ASSERT_IS_TRUE(ok);
        ok = berdy.serializeSensorVariables(sensMeas,
                                            extWrenches,
                                            genTrqs.jointTorques(),
                                            generalizedProperAccs.jointAcc(),
                                            intWrenches,
...
{
    std::vector<iDynTree::BerdyDynamicVariable> dynVarOrdering
        = berdy.getDynamicVariablesOrdering();
...
void testBerdyOriginalFixedBase(BerdyHelper& berdy, std::string filename)
{
    ...
    // Get the angular v
    berdy.updateKinematicsFromFixedBase(pos.jointPos(),
                                        vel.jointVel(),
    ...
    */
    ...
    if (berdy.getNrOfSensorsMeasurements() > 0)
    {
        ...
        y.zero();
        SensorsMeasurements sensMeas(berdy.sensors());
        ok = predictSensorsMeasurementsFromRawBuffers(berdy.model(),
                                                      berdy.sensors(),
                                                      berdy.dynamicTraversal(),
                                                      linkVels,
                                                      linkProperAccs,
        ...
        std::cerr << Y.description(true) << std::endl;
        std::cerr << "Testing " << berdy.getOptions().jointOnWhichTheInternalWrenchIsMeasured[0] <<
        std::endl; std::cerr <<
        intWrenches(berdy.model().getJointIndex(berdy.getOptions().jointOnWhichTheInternalWrenchIsMeasured[0])).toString()
        << std::endl;
        */
...
void testBerdyHelpers(std::string fileName)
{
    ...
    if (estimator.model().getNrOfJoints() > 0)
    {
        ...
        options.jointOnWhichTheInternalWrenchIsMeasured.push_back(
            estimator.model().getJointName(jntIdx));
    }
    ...
    if (ok)
    {
        std::cerr << "Testing ORIGINAL_BERDY_FIXED_BASE tests for model " << fileName
                  << " because the assumptions of ORIGINAL_BERDY_FIXED_BASE are respected"
                  << std::endl;
    ...
    {
        std::cerr << "Skipping ORIGINAL_BERDY_FIXED_BASE tests for model " << fileName
                  << " because some assumptions of ORIGINAL_BERDY_FIXED_BASE are not respected"
                  << std::endl;
    }
...
int main()
{
    for (unsigned int mdl = 0; mdl < IDYNTREE_TESTS_URDFS_NR; mdl++)
    {
        ...
        std::cout << "BerdyHelperUnitTest, testing file " << std::string(IDYNTREE_TESTS_URDFS[mdl])
                  << std::endl;
