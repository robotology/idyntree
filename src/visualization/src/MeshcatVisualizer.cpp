...
struct MeshcatVisualizer::Impl
{
    ...
    bool addModelGeometryToView(MultiBodyModelData& data, const std::string& modelName)
    {
                                                 ...
                                                 linkPositions))
        {
            const std::string msg
                = "Unable tos solve the inverse kinematics for the model named " + modelName;
        ...
        for (const auto& linkSolidShapes : linksSolidShapes)
        {
            ...
            for (auto linkSolidShape : linkSolidShapes)
            {
                ...
                const std::string viewerName
                    = this->getMeshPathInMeshcatTree(modelName, linkName, *externalMesh);
                             ...
                             const std::string& modelName)
    {
        ...
        {
            const std::string msg = "Wrong size of the jointPositions vector for the model "
                                    + modelName
        ...
        {
            const std::string msg
                = "Unable to solve the inverse kinematics for the model named " + modelName;
            reportError("MeshcatVisualizer::Impl", "updateModelGeometry", msg.c_str());
            return false;
        }
...
bool MeshcatVisualizer::loadModel(const iDynTree::Model& model, const std::string& modelName)
{
                                   ...
                                   const std::string& name)
{
    ...
    if (color.size() != 4)
    {
        const std::string msg = "The color needs to be a vector of 4 elements between 0 and 1. "
                                "Provided: "
...
bool MeshcatVisualizer::loadCylinder(const double radius,
                                     const double height,
                                     const iDynTree::Span<const double>& color,
                                     const std::string& name)
{
...
bool MeshcatVisualizer::loadBox(const double width,
                                const double depth,
                                const double height,
                                const iDynTree::Span<const double>& color,
                                const std::string& name)
...
bool MeshcatVisualizer::loadEllipsoid(const double a,
                                      const double b,
                                      const double c,
                                      const iDynTree::Span<const double>& color,
                                      const std::string& name)
...
{
    return this->setPrimitiveGeometryTransform(iDynTree::make_matrix_view(
                                                   world_T_geometry.asHomogeneousTransform()),
                                               geometryName);
}

bool MeshcatVisualizer::setPrimitiveGeometryTransform(
    const iDynTree::MatrixView<const double>& world_T_geometry, const std::string& geometryName)
{
    if (world_T_geometry.rows() != world_T_geometry.cols() || world_T_geometry.rows() != 4)
    {
        const std::string msg = "world_T_geometry needs to be a 4x4 matrix. Provided a "
                                + std::to_string(world_T_geometry.rows()) + "x"
                                + std::to_string(world_T_geometry.cols()) + " matrix.";
