                             ...
                             const VisualizerOptions& textureOptions)
{
    irrTexture
        = irrDriverInput
              ->addRenderTargetTexture(irr::core::dimension2d<irr::u32>(textureOptions.winWidth,
                                                                        textureOptions.winHeight),
                                       name.c_str());
...
iDynTree::ColorViz iDynTree::Texture::getPixelColor(unsigned int width, unsigned int height) const
{
    ...
    if (!irrTexture)
    {
        reportError("Texture",
                    ...
                    "Cannot get pixel color. The video texture has not been properly initialized.");
        return pixelOut;
    }
    ...
    unsigned char* buffer
        = (unsigned char*)irrTexture->lock(irr::video::E_TEXTURE_LOCK_MODE::ETLM_READ_ONLY);
    if (buffer)
    {
        pixelIrrlicht
            = irr::video::SColor(*(unsigned int*)(buffer + (height * pitch) + (width * bytes)));
        irrTexture->unlock();
    }
...
bool iDynTree::Texture::getPixels(std::vector<iDynTree::PixelViz>& pixels) const
{
    ...
    if (buffer)
    {
        ...
        for (size_t width = 0; width < textureDim.Width; ++width)
        {
            ...
            {
                pixelIrrlicht = irr::video::SColor(
                    *(unsigned int*)(buffer + (height * pitch) + (width * bytes)));
                irr::video::SColorf pixelIrrlichtFloat(pixelIrrlicht);
...
bool iDynTree::Texture::drawToFile(const std::string filename) const
{
    if (!irrTexture)
    {
        reportError("Texture",
                    "drawToFile",
                    "Cannot save the texture to file. It has not properly initialized.");
        return false;
    }

    auto textureDim = irrTexture->getSize();
    irr::video::IImage* const image
        = irrDriver->createImage(irrTexture, irr::core::position2d<irr::s32>(0, 0), textureDim);
    bool retValue = false;
...
bool iDynTree::Texture::setSubDrawArea(int xOffsetFromTopLeft,
                                       ...
                                       int subImageHeight)
{
    ...
    if (xOffsetFromTopLeft + subImageWidth > width())
    {
        reportError("Texture",
                    ...
                    "xOffsetFromTopLeft and width are greater than the texture width.");
        return false;
    }
    ...
    if (yOffsetFromTopLeft + subImageHeight > height())
    {
        reportError("Texture",
                    "setSubDrawArea",
                    "The specified draw coordinates are out of bounds. The sum of the "
                    "yOffsetFromTopLeft and height are greater than the texture height.");
        return false;
    }
