...
void iDynTree::TexturesHandler::draw(iDynTree::Environment& defaultEnvironment,
                                     iDynTree::Camera& defaultCamera,
                                     bool clearBuffers)
{
    ...
    for (auto t = m_textures.begin(); t != m_textures.end(); ++t)
    {
        ...
        {
            t->second->textureEnvironment.m_envNode->setVisible(true); // Enable the texture
                                                                       // environment
            bool clearThisTexture = clearBuffers || t->second->forceClear;
            // set render target texture
            m_irrDriver->setRenderTarget(t->second->irrTexture,
                                         clearThisTexture,
                                         clearThisTexture,
                                         t->second->textureEnvironment.m_backgroundColor.toSColor());

            m_irrDriver->setViewPort(irr::core::rect<irr::s32>(
                0,
                0,
                t->second->irrTexture->getSize().Width,
                t->second->irrTexture->getSize()
                    .Height)); // workaround for
                               // http://irrlicht.sourceforge.net/forum/viewtopic.php?f=7&t=47004
            m_irrDriver->setViewPort(t->second->viewport); // Setting the viewport specified for the
                                                           // texture
            float width
                = t->second->viewport.LowerRightCorner.X - t->second->viewport.UpperLeftCorner.X;
            float height
                = t->second->viewport.LowerRightCorner.Y - t->second->viewport.UpperLeftCorner.Y;
            defaultCamera.setAspectRatio(width / height);

            // draw whole scene into render buffer
            m_sceneManager->drawAll();

            t->second->textureEnvironment.m_envNode->setVisible(false); // Disable the texture
                                                                        // environment
        }
    }
    ...
    m_irrDriver->setRenderTarget(0,
                                 clearBuffers,
                                 clearBuffers,
                                 defaultEnvironment.m_backgroundColor.toSColor());
}

iDynTree::ITexture*
iDynTree::TexturesHandler::add(const std::string& name,
                               const iDynTree::VisualizerOptions& textureOptions)
{
    if (!m_irrDriver || !m_sceneManager)
    {
        reportError("TexturesHandler",
                    "add",
                    "The handler has not been initialized yet. Call the init() method first.");
        return nullptr;
    }
    ...
    if (!m_areTexturesSupported)
    {
        reportError("TexturesHandler",
                    "add",
                    "The video driver does not support rendering to a target.");
        return nullptr;
    }
