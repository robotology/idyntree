...
ModelParserOptions::ModelParserOptions()
    : addSensorFramesAsAdditionalFrames(true), originalFilename(""),
...
bool ModelLoader::ModelLoaderPimpl::setModel(const Model &_model) {
  ...
  if (!m_isModelValid) {
    reportError("ModelLoader", "setModel",
                "Loading failed, resetting ModelLoader to be invalid");
    m_model = Model();
  }
...
ModelLoader::ModelLoader() : m_pimpl(new ModelLoaderPimpl()) {
  m_pimpl->m_isModelValid = false;
}

ModelLoader::~ModelLoader() {}

const Model &ModelLoader::model() { return m_pimpl->m_model; }

const SensorsList &ModelLoader::sensors() { return m_pimpl->m_model.sensors(); }

bool ModelLoader::isValid() { return m_pimpl->m_isModelValid; }

const ModelParserOptions &ModelLoader::parsingOptions() const {
...
bool ModelLoader::loadModelFromFile(
  ...
  std::shared_ptr<const URDFDocument> urdfDocument =
      std::dynamic_pointer_cast<const URDFDocument>(document);
  if (!urdfDocument) {
    reportError("ModelLoader", "loadModelFromFile",
                "Fatal error in retrieving the parsed model.");
    return false;
  }
...
bool ModelLoader::loadModelFromString(
    const std::string &modelString, const std::string &filetype,
    const std::vector<std::string> &packageDirs /* = {} */) {
...
bool ModelLoader::loadReducedModelFromFullModel(
    const Model &fullModel, const std::vector<std::string> &consideredJoints,
    const std::string filetype) {
  std::unordered_map<std::string, std::vector<double>>
      emptyRemovedJointPositions;
  return this->loadReducedModelFromFullModel(
      fullModel, consideredJoints, emptyRemovedJointPositions, filetype);
}

bool ModelLoader::loadReducedModelFromString(
  ...
  std::unordered_map<std::string, std::vector<double>>
      emptyRemovedJointPositions;
  return this->loadReducedModelFromString(modelString, consideredJoints,
                                          emptyRemovedJointPositions, filetype,
                                          packageDirs);
}

bool ModelLoader::loadReducedModelFromFile(
  ...
  std::unordered_map<std::string, std::vector<double>>
      emptyRemovedJointPositions;
  return this->loadReducedModelFromFile(filename, consideredJoints,
                                        emptyRemovedJointPositions, filetype,
                                        packageDirs);
}
...
bool ModelLoader::loadReducedModelFromFullModel(
  ...
  return loadReducedModelFromFullModel(fullModel, consideredJoints,
                                       removedJointPositionsVector, filetype);
}
...
bool ModelLoader::loadReducedModelFromString(
  ...
  return loadReducedModelFromString(modelString, consideredJoints,
                                    removedJointPositionsVector, filetype,
                                    packageDirs);
}
...
bool ModelLoader::loadReducedModelFromFile(
  ...
  return loadReducedModelFromFile(filename, consideredJoints,
                                  removedJointPositionsVector, filetype,
                                  packageDirs);
}
} // namespace iDynTree
