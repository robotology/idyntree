...
SpatialInertia boxGet6DInertiaInLinkFrameFromDensity(const Box& box, double density)
{
    ...
    SpatialInertia inertiaInGeometryFrame
        = SpatialInertia(boxMass, comInGeomFrame, rotInertiaInGeomFrame);
...
// Inspired  from
// https://github.com/ros-visualization/rviz/blob/070835c426b8982e304b38eb4a9c6eb221155d5f/src/rviz/mesh_loader.cpp#L644
                            ...
                            const std::vector<Position>& vertexVector)
{
    ...
    box.setLink_H_geometry(Transform(link_H_vertices.getRotation(), offset_bb_wrt_link));
                                    ...
                                    std::vector<iDynTree::Box>& linkBBsInLinkFrame)
{
    ...
    for (LinkIndex lnkIdx = 0; lnkIdx < model.getNrOfLinks(); lnkIdx++)
    {
        ...
        for (int shapeIdx = 0;
             shapeIdx < model.collisionSolidShapes().getLinkSolidShapes()[lnkIdx].size();
             shapeIdx++)
        {
...
std::vector<Position> computeBoxVertices(const Box box)
{
    ...
    // + + +
    vertices.push_back(box.getLink_H_geometry()
                       * Position(+box.getX() / 2, +box.getY() / 2, +box.getZ() / 2));
...
{
    // Extract BB from shape, this would be benefic from being moved as a method in the SolidShape
    // interface
    if (geom.isBox())
    {
    ...
    reportError("",
                ...
                "IDYNTREE_USES_ASSIMP CMake option need to be set to ON to use "
                "computeBoundingBoxFromShape");
#endif
    return false;
}
    ...
    const double totalMass, iDynTree::Model& model, VectorDynSize& estimatedInertialParams)
{
    ...
    {
        linkInertias[lnkIdx]
            = boxGet6DInertiaInLinkFrameFromDensity(boundingBoxVolume[lnkIdx], density);
    }
