...
template <typename> struct is_matrixfixsize : std::false_type
{
};
...
const std::vector<size_t>& SparsityHelper::NullIndicesVector()
{
    return s_nullVector;
}
...
bool SparsityHelper::addConstraintSparsityPatternTemplated(
    const MatrixType& newConstraint, const iDynTree::IndexRange& constraintRange)
{
    static_assert(std::is_base_of<MatrixType, iDynTree::MatrixDynSize>::value
                      || is_matrixfixsize<MatrixType>::value,
                  "addConstraintSparsityPatternTemplated can be called only with "
                  "iDynTree::MatrixFixSize and iDynTree::MatrixDynSize");
    ...
    m_numberOfNonZeros.resize(1 + previousConstraintSize + constraintSize,
                              m_numberOfNonZeros[previousConstraintSize]);
    m_nonZeroIndices.resize(previousConstraintSize + constraintSize);
    for (size_t row = 0; row < constraintSize; ++row)
    {
        size_t numberOfNonZeros = static_cast<size_t>(
            iDynTree::toEigen(newConstraint).row(constraintRange.offset + row).sum());
        m_numberOfNonZeros[1 + previousConstraintSize + row]
            = totalNumberOfNonZerosBeforeRow(previousConstraintSize + row) + numberOfNonZeros;
        ...
        {
            if (std::abs(newConstraint(constraintRange.offset + row, col) - 1)
...
{
    return addConstraintSparsityPattern(newConstraint,
                                        {0, static_cast<ptrdiff_t>(newConstraint.rows())});
}

template <unsigned int nRows, unsigned int nCols>
bool SparsityHelper::addConstraintSparsityPattern(
    const iDynTree::MatrixFixSize<nRows, nCols>& newConstraint)
{
    return addConstraintSparsityPattern(newConstraint,
                                        {0, static_cast<ptrdiff_t>(newConstraint.rows())});
}
...
{
    if (rowIndex >= m_numberOfNonZeros.size() - 1)
        return 0;
    return m_numberOfNonZeros[rowIndex + 1] - m_numberOfNonZeros[rowIndex];
}
size_t SparsityHelper::numberOfNonZeros() const
{
    return m_numberOfNonZeros[m_numberOfNonZeros.size() - 1];
}
...
{
    if (rowIndex > m_nonZeroIndices.size())
        return s_nullVector;
                                              ...
                                              Ipopt::Number* outputBuffer)
{
    for (size_t row = 0; row < constraintRange.size; ++row)
    {
        ...
        const std::vector<size_t>& currentSparsityRow
            = nonZeroIndicesForRow(constraintRange.offset + row);
        Ipopt::Number* startingValue
            = &outputBuffer[totalNumberOfNonZerosBeforeRow(constraintRange.offset + row)];

        for (size_t colIndex = 0; colIndex < currentSparsityRow.size(); ++colIndex)
        {
            startingValue[colIndex]
                = fullMatrix(fullMatrixStartingRowIndex + row, currentSparsityRow[colIndex]);
        }
    }
}
...
void InverseKinematicsNLP::initializeInternalData()
{
    ...
    // resize some buffers
    finalJacobianBuffer.resize((3
                                + sizeOfRotationParametrization(m_data.m_rotationParametrization)),
                               3 + sizeOfRotationParametrization(m_data.m_rotationParametrization)
                                   + m_data.m_dofs);
    ...
    for (TransformMap::const_iterator constraint = m_data.m_constraints.begin();
         constraint != m_data.m_constraints.end();
         ++constraint)
    {
        FrameInfo info;
        info.jacobian.resize(6, m_data.m_dofs + 6);
    ...
    comInfo.comJacobianAnalytical.resize(3,
                                         m_data.m_dofs + 3
                                             + sizeOfRotationParametrization(
                                                 m_data.m_rotationParametrization));
    comInfo.projectedComJacobian.resize(m_data.m_comHullConstraint.getNrOfConstraints(),
                                        m_data.m_dofs + 3
                                            + sizeOfRotationParametrization(
                                                m_data.m_rotationParametrization));
...
void InverseKinematicsNLP::addSparsityInformationForConstraint(
    int constraintID, const internal::kinematics::TransformConstraint& constraint)
{
    // For each constraint compute its jacobian pattern
    FrameInfo& constraintInfo = constraintsInfo[constraintID];
    // iDynTree pattern
    m_data.dynamics().getFrameFreeFloatingJacobianSparsityPattern(constraintID,
                                                                  constraintInfo.jacobian);
    ...
    for (unsigned row = 0; row < finalJacobianBuffer.rows(); ++row)
    {
        ...
        {
            finalJacobianBuffer(row, col)
                = std::abs(finalJacobianBuffer(row, col)) < iDynTree::DEFAULT_TOL ? 0.0 : 1.0;
        }
    }
    ...
    {
        // Orientation part
        m_jacobianSparsityHelper
            .addConstraintSparsityPattern(finalJacobianBuffer,
                                          {3,
                                           sizeOfRotationParametrization(
                                               m_data.m_rotationParametrization)});
    }
...
void InverseKinematicsNLP::initializeSparsityInformation()
{
    ...
    if (m_data.m_comHullConstraint.isActive()
        || (m_data.isCoMTargetActive() && m_data.isCoMaConstraint()))
    {
        // TODO: implement quaternion part
        assert(m_data.m_rotationParametrization
               == iDynTree::InverseKinematicsRotationParametrizationRollPitchYaw);
         ...
         ++target)
    {
        ...
        if (target->second.isActive())
        {
            ...
            if (target->second.targetResolutionMode()
                & iDynTree::InverseKinematicsTreatTargetAsConstraintPositionOnly)
                computationOption |= ComputeContraintJacobianOptionLinearPart;
            if (target->second.targetResolutionMode()
                & iDynTree::InverseKinematicsTreatTargetAsConstraintRotationOnly)
                computationOption |= ComputeContraintJacobianOptionAngularPart;

            if (computationOption == 0)
                continue; // no need for further computations
...
bool InverseKinematicsNLP::updateState(const Ipopt::Number* x)
{
    ...
    Eigen::Map<const Eigen::VectorXd>
        optimisedJoints(&x[3 + sizeOfRotationParametrization(m_data.m_rotationParametrization)],
                        m_data.m_dofs);
        ...
        == iDynTree::InverseKinematicsRotationParametrizationQuaternion)
    {
        ...
        Eigen::Map<Eigen::Matrix<double, 3, 4, Eigen::RowMajor>> inverseMap
            = iDynTree::toEigen(quaternionDerivativeInverseMapBuffer);
        ...
        inverseMap.rightCols<3>()
            += iDynTree::skew(iDynTree::toEigen(normQuaternioniDyn).tail<3>());
         ...
         * \f[
         *  \frac{\partial z_f}{\partial \bar{z}_B} &= \frac{\partial z_f}{\partial z_B}
         * \frac{\partial z_B}{\partial \bar{z}_B}
         * \f]
    ...
    {
        // RPY parametrization
        baseOrientationRotation = iDynTree::Rotation::RPY(this->optimizedBaseOrientation(0),
                                                          this->optimizedBaseOrientation(1),
                                                          this->optimizedBaseOrientation(2));
    }
    // base pose
         ...
         ++target)
    {
        ...
        Eigen::Map<Eigen::Matrix<double, 4, 3, Eigen::RowMajor>> map
            = iDynTree::toEigen(frameInfo.quaternionDerivativeMap);
...
bool InverseKinematicsNLP::get_bounds_info(Ipopt::Index n,
                                           Ipopt::Number* x_l,
                                           Ipopt::Number* x_u,
                                           Ipopt::Index m,
                                           Ipopt::Number* g_l,
                                           Ipopt::Number* g_u)
{
    ...
    for (TransformMap::const_iterator it = m_data.m_targets.begin(); it != m_data.m_targets.end();
         ++it)
    {
        if (it->second.targetResolutionMode()
                & iDynTree::InverseKinematicsTreatTargetAsConstraintPositionOnly
...
bool InverseKinematicsNLP::get_starting_point(Ipopt::Index n,
                                              bool init_x,
                                              Ipopt::Number* x,
                                              bool init_z,
                                              Ipopt::Number* z_L,
                                              ...
                                              Ipopt::Number* lambda)
{
    ...
    if (init_x)
    {
        ...
        Eigen::Map<Eigen::VectorXd> baseRotation(&x[3],
                                                 sizeOfRotationParametrization(
                                                     m_data.m_rotationParametrization));
        ...
        case iDynTree::InverseKinematicsRotationParametrizationQuaternion:
            baseRotation
                = iDynTree::toEigen(m_data.m_baseInitialCondition.getRotation().asQuaternion());
            break;
        case iDynTree::InverseKinematicsRotationParametrizationRollPitchYaw:
            baseRotation.head<3>()
                = iDynTree::toEigen(m_data.m_baseInitialCondition.getRotation().asRPY());
            break;
        default:
            return false;
        }
...
bool InverseKinematicsNLP::eval_f(Ipopt::Index n,
                                  ...
                                  Ipopt::Number& obj_value)
{
    ...
    Eigen::VectorXd jointError = iDynTree::toEigen(jointsAtOptimisationStep)
                                 - iDynTree::toEigen(m_data.m_preferredJointsConfiguration);
         ...
         ++target)
    {
        ...
        if ((target->second.targetResolutionMode()
                 == iDynTree::InverseKinematicsTreatTargetAsConstraintRotationOnly
             || target->second.targetResolutionMode()
            ...
            && target->second.hasPositionConstraint())
        {
            ...
            iDynTree::Position positionError
                = targetsInfo[target->first].transform.getPosition() - target->second.getPosition();
            obj_value += 0.5 * target->second.getPositionWeight()
                         * (iDynTree::toEigen(positionError)).squaredNorm();
        }
        if ((target->second.targetResolutionMode()
                 == iDynTree::InverseKinematicsTreatTargetAsConstraintPositionOnly
             || target->second.targetResolutionMode()
            ...
            && target->second.hasRotationConstraint())
        {
                ...
                == iDynTree::InverseKinematicsRotationParametrizationQuaternion)
            {
                ...
                // orientation error, as  w_R_f * (w_R_f^d)^{-1} = w_\tilde{R}_w (R tilde expressed
                // in inertial)
                // TODO: check if it is correct to express it in the inertial
                iDynTree::Rotation transformError
                    = targetsInfo[target->first].transform.getRotation()
                      * target->second.getRotation().inverse();
                ...
                obj_value += 0.5 * target->second.getRotationWeight()
                             * (iDynTree::toEigen(orientationErrorQuaternion)
                                - iDynTree::toEigen(identityQuaternion))
                                   .squaredNorm();
            } else if (m_data.m_rotationParametrization
                       == iDynTree::InverseKinematicsRotationParametrizationRollPitchYaw)
            {
                ...
                iDynTree::toEigen(rotation_error) = iDynTree::toEigen(rotation_desired).transpose()
                                                    * iDynTree::toEigen(rotation_target);

                // TODO Investigate the derivative of the cost using the RPY representation of the
                // error matrix R*\hat{R}'
                obj_value += 0.5 * target->second.getRotationWeight()
                             * iDynTree::toEigen(rotation_error.asRPY()).squaredNorm();
            }
        }
    }
    ...
    if (m_data.isCoMTargetActive() && !(m_data.isCoMaConstraint()))
    {
        ...
        obj_value
            += 0.5 * m_data.m_comTarget.weight * iDynTree::toEigen(comPositionError).squaredNorm();
    }

    return true;
}

bool InverseKinematicsNLP::eval_grad_f(Ipopt::Index n,
                                       const Ipopt::Number* x,
                                       bool new_x,
                                       Ipopt::Number* grad_f)
{
         ...
         ++target)
    {
            ...
            && target->second.hasPositionConstraint())
        {
                       ...
                       == iDynTree::InverseKinematicsRotationParametrizationRollPitchYaw)
            {
                // RPY parametrization for the base
                iDynTree::Vector3 rpy;
                iDynTree::toEigen(rpy)
                    = iDynTree::toEigen(this->optimizedBaseOrientation).head<3>();
                iDynTree::Matrix3x3 RPYToOmega
                    = iDynTree::Rotation::RPYRightTrivializedDerivative(rpy(0), rpy(1), rpy(2));

                // RPY parametrization for the constraint
                iDynTree::Vector3 rpy_target
                    = targetsInfo[target->first].transform.getRotation().asRPY();
                iDynTree::Matrix3x3 omegaToRPYMap_target
                    = iDynTree::Rotation::RPYRightTrivializedDerivativeInverse(rpy_target(0),
                                                                               rpy_target(1),
                                                                               rpy_target(2));
            ...
            gradient += target->second.getPositionWeight()
                        * iDynTree::toEigen(positionError).transpose()
                        * iDynTree::toEigen(finalJacobianBuffer).topRows<3>();
        }
        if ((target->second.targetResolutionMode()
                 == iDynTree::InverseKinematicsTreatTargetAsConstraintPositionOnly
             || target->second.targetResolutionMode()
            ...
            && target->second.hasRotationConstraint())
        {
                ...
                == iDynTree::InverseKinematicsRotationParametrizationQuaternion)
            {
                ...
                gradient += target->second.getRotationWeight()
                            * (iDynTree::toEigen(orientationErrorQuaternion)
                               - iDynTree::toEigen(identityQuaternion))
                                  .transpose()
                            * iDynTree::toEigen(finalJacobianBuffer)
                                  .bottomRows(sizeOfRotationParametrization(
                                      m_data.m_rotationParametrization));
            } else if (m_data.m_rotationParametrization
                       == iDynTree::InverseKinematicsRotationParametrizationRollPitchYaw)
            {
                ...
                iDynTree::Matrix3x3 omegaToRPYMap_target;
                iDynTree::toEigen(omegaToRPYMap_target)
                    = iDynTree::toEigen(
                          iDynTree::Rotation::RPYRightTrivializedDerivativeInverse(rpy_error(0),
                                                                                   rpy_error(1),
                                                                                   rpy_error(2)))
                      * iDynTree::toEigen(rotation_desired).transpose();
                ...
                // error matrix R*\hat{R}'
                gradient += target->second.getRotationWeight()
                            * (iDynTree::toEigen(rpy_error)).transpose()
                            * iDynTree::toEigen(finalJacobianBuffer)
                                  .bottomRows(sizeOfRotationParametrization(
                                      m_data.m_rotationParametrization));
            }
    ...
    if (m_data.isCoMTargetActive() && !(m_data.isCoMaConstraint()))
    {
        ...
        iDynTree::Matrix3x3 RPYToOmega
            = iDynTree::Rotation::RPYRightTrivializedDerivative(rpy(0), rpy(1), rpy(2));
        computeConstraintJacobianCOMRPY(comInfo.comJacobian,
                                        RPYToOmega,
                                        comInfo.comJacobianAnalytical);

        iDynTree::Position comPositionError;
        comPositionError = comInfo.com - m_data.m_comTarget.desiredPosition;
        gradient += m_data.m_comTarget.weight * iDynTree::toEigen(comPositionError).transpose()
                    * iDynTree::toEigen(comInfo.comJacobianAnalytical);
    }

    return true;
}

bool InverseKinematicsNLP::eval_g(
    Ipopt::Index n, const Ipopt::Number* x, bool new_x, Ipopt::Index m, Ipopt::Number* g)
{
        ...
        if (constraint->second.isActive())
        {
            ...
            if (constraint->second.hasRotationConstraint())
            {
                           ...
                           == iDynTree::InverseKinematicsRotationParametrizationRollPitchYaw)
                {
                    ...
                    currentRotation.getRPY(constraints(index),
                                           constraints(index + 1),
                                           constraints(index + 2));
                }
    ...
    if (m_data.m_comHullConstraint.isActive())
    {
        constraints.segment(index, m_data.m_comHullConstraint.getNrOfConstraints())
            = iDynTree::toEigen(m_data.m_comHullConstraint.A)
...
bool InverseKinematicsNLP::eval_jac_g(Ipopt::Index n,
                                      const Ipopt::Number* x,
                                      bool new_x,
                                      Ipopt::Index m,
                                      ...
                                      Ipopt::Number* values)
{
    if (!values)
    {
        ...
        for (Ipopt::Index row = 0; row < m; row++)
        {
            ...
            size_t numberOfPreviousNonzeros
                = m_jacobianSparsityHelper.totalNumberOfNonZerosBeforeRow(row);
    ...
    } else
    {
             ...
             ++constraint)
        {
            ...
            if (constraint->second.isActive())
            {
                ...
                if (constraint->second.hasRotationConstraint())
                {
                    // Orientation part
                    m_jacobianSparsityHelper
                        .assignActualMatrixValues({constraintIndex,
                                                   sizeOfRotationParametrization(
                                                       m_data.m_rotationParametrization)},
                                                  finalJacobianBuffer,
                                                  3,
            ...
            || (m_data.isCoMTargetActive() && m_data.isCoMaConstraint()))
        {
            ...
            if (m_data.m_comHullConstraint.isActive())
            {
                ...
                iDynTree::toEigen(comInfo.projectedComJacobian)
                    = iDynTree::toEigen(m_data.m_comHullConstraint.A)
                      * iDynTree::toEigen(m_data.m_comHullConstraint.Pdirection)
                      * iDynTree::toEigen(comInfo.comJacobianAnalytical);

                m_jacobianSparsityHelper
                    .assignActualMatrixValues({constraintIndex,
                                               static_cast<ptrdiff_t>(
                                                   comInfo.projectedComJacobian.rows())},
            ...
            if (m_data.isCoMTargetActive())
            {
                m_jacobianSparsityHelper
                    .assignActualMatrixValues({constraintIndex,
                                               static_cast<ptrdiff_t>(
                                                   comInfo.comJacobianAnalytical.rows())},
                                              comInfo.comJacobianAnalytical,
                                              0,
                                              values);
                constraintIndex += 3;
            ...
            == iDynTree::InverseKinematicsRotationParametrizationQuaternion)
        {
            ...
            quaternionDerivative
                = 2 * iDynTree::toEigen(this->optimizedBaseOrientation).transpose();
            constraintIndex++;
        }
...
bool InverseKinematicsNLP::eval_h(Ipopt::Index n,
                                  const Ipopt::Number* x,
                                  bool new_x,
...
void InverseKinematicsNLP::finalize_solution(Ipopt::SolverReturn status,
                                             Ipopt::Index n,
                                             const Ipopt::Number* x,
...
{
    /*
     This method is only important if the limited-memory quasi-Newton options is used, see
     Section 4.2. It is used to return the number of variables that appear nonlinearly in the
     objective function or in at least one constraint function. If a negative number is returned,
     IPOPT assumes that all variables are nonlinear.

     If the user doesn't overload this method in her implementation of the class derived from TNLP,
...
{
    /*
     This method is called by IPOPT only if the limited-memory quasi-Newton options is used and if
     the get_number_of_nonlinear_variables method returns a positive number; this number is then
     identical with num_nonlin_vars and the length of the array pos_nonlin_vars. In this call, you
     need to list the indices of all nonlinear variables in pos_nonlin_vars, where the numbering
     starts with 0 order 1, depending on the numbering style determined in get_nlp_info.
     */
    return true;
}

void InverseKinematicsNLP::computeConstraintJacobian(
    const iDynTree::MatrixDynSize& transformJacobianBuffer,
...
{
    Eigen::Map<const Eigen::Matrix<double, 4, 3, Eigen::RowMajor>> quaternionDerivativeMap
        = iDynTree::toEigen(_quaternionDerivativeMap);
    Eigen::Map<const Eigen::Matrix<double, 3, 4, Eigen::RowMajor>> quaternionDerivativeInverseMap
        = iDynTree::toEigen(_quaternionDerivativeInverseMap);
    ...
    iDynTree::iDynTreeEigenConstMatrixMap frameJacobian
        = iDynTree::toEigen(transformJacobianBuffer);
    iDynTree::iDynTreeEigenMatrixMap constraintJacobian
        = iDynTree::toEigen(constraintJacobianBuffer);
    ...
    if (computationOption & ComputeContraintJacobianOptionLinearPart)
    {
        ...
        constraintJacobian.block<3, 4>(0, 3)
            = frameJacobian.block<3, 3>(0, 3) * quaternionDerivativeInverseMap;
    ...
    if (computationOption & ComputeContraintJacobianOptionAngularPart)
    {
        ...
        constraintJacobian.bottomLeftCorner<4, 3>()
            = quaternionDerivativeMap * frameJacobian.bottomLeftCorner<3, 3>();
        constraintJacobian.block<4, 4>(3, 3) = quaternionDerivativeMap
                                               * frameJacobian.block<3, 3>(3, 3)
                                               * quaternionDerivativeInverseMap;
        constraintJacobian.bottomRightCorner(4, n)
            = quaternionDerivativeMap * frameJacobian.bottomRightCorner(3, n);
    }
}

void InverseKinematicsNLP::computeConstraintJacobianRPY(
    const iDynTree::MatrixDynSize& transformJacobianBuffer,
    ...
    iDynTree::MatrixDynSize& constraintJacobianBuffer)
{
    Eigen::Map<const Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> rpyDerivativeMap
        = iDynTree::toEigen(_rpyDerivativeMap);
    Eigen::Map<const Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> rpyDerivativeInverseMap
        = iDynTree::toEigen(_rpyDerivativeInverseMap);
    ...
    if (computationOption & ComputeContraintJacobianOptionLinearPart)
    {
        // Position (linear) part of the Jacobian
        constraintJacobian.topLeftCorner<3, 3>() = frameJacobian.topLeftCorner<3, 3>();
        constraintJacobian.block<3, 3>(0, 3)
            = frameJacobian.block<3, 3>(0, 3) * rpyDerivativeInverseMap;
        constraintJacobian.topRightCorner(3, n) = frameJacobian.topRightCorner(3, n);
    }
    ...
    {
        // Angular part of the Jacobian
        constraintJacobian.bottomLeftCorner<3, 3>()
            = rpyDerivativeMap * frameJacobian.bottomLeftCorner<3, 3>();
        constraintJacobian.block<3, 3>(3, 3)
            = rpyDerivativeMap * frameJacobian.block<3, 3>(3, 3) * rpyDerivativeInverseMap;
        constraintJacobian.bottomRightCorner(3, n)
            = rpyDerivativeMap * frameJacobian.bottomRightCorner(3, n);
    }
}

void InverseKinematicsNLP::computeConstraintJacobianCOMRPY(
    const iDynTree::MatrixDynSize& comJacobianBuffer,
    const iDynTree::MatrixFixSize<3, 3>& _rpyDerivativeInverseMap,
    iDynTree::MatrixDynSize& constraintComJacobianBuffer)
{
    ...
    iDynTree::iDynTreeEigenMatrixMap constraintJacobian
        = iDynTree::toEigen(constraintComJacobianBuffer);
                                                ...
                                                iDynTree::Matrix3x3& map)
{
    ...
    /*
     inverse is
     [                                                                              cos(psi),
     sin(psi),                                             0] [ -(sin(psi)*(cos(psi)^2*cos(theta) +
     cos(theta)*sin(psi)^2))/(cos(psi)^2 + sin(psi)^2), (cos(psi)*(cos(psi)^2*cos(theta) +
     cos(theta)*sin(psi)^2))/(cos(psi)^2 + sin(psi)^2), 0] [ -cos(psi)*sin(theta),
     -sin(psi)*sin(theta), cos(psi)^2*cos(theta) + cos(theta)*sin(psi)^2]
     */
...
void InverseKinematicsNLP::testDerivatives(const iDynTree::VectorDynSize& derivativePoint,
                                           int frameIndex,
                                           double epsilon,
                                           double tolerance,
                                           int _parametrization)
{
    ...
    enum iDynTree::InverseKinematicsRotationParametrization parametrization
        = (enum iDynTree::InverseKinematicsRotationParametrization)_parametrization;
    ...
    iDynTree::MatrixDynSize _analyticalJacobian(3
                                                    + iDynTree::sizeOfRotationParametrization(
                                                        parametrization),
                                                derivativePoint.size());
    _analyticalJacobian.zero();
    Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> analyticalJacobian
        = toEigen(_analyticalJacobian);

    updateState(_x);

    iDynTree::MatrixDynSize dynTreeJacobian(6,
                                            6 + derivativePoint.size()
                                                - (3
                                                   + iDynTree::sizeOfRotationParametrization(
                                                       parametrization)));
    ...
    iDynTree::MatrixDynSize _finiteDifferenceJacobian(3
                                                          + iDynTree::sizeOfRotationParametrization(
                                                              parametrization),
                                                      derivativePoint.size());
    _finiteDifferenceJacobian.zero();
    Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>
        finiteDifferenceJacobian = toEigen(_finiteDifferenceJacobian);
    ...
    std::cerr << analyticalJacobian.leftCols(
        3 + iDynTree::sizeOfRotationParametrization(parametrization))
              << "\n\n";
    std::cerr << "Finite Difference Jacobian (Base)\n";
    std::cerr << finiteDifferenceJacobian.leftCols(
        3 + iDynTree::sizeOfRotationParametrization(parametrization))
              << "\n\n\n";
    std::cerr << "Analytical Jacobian (Joints)\n";
    std::cerr << analyticalJacobian.rightCols(
        derivativePoint.size() - (3 + iDynTree::sizeOfRotationParametrization(parametrization)))
              << "\n\n";
    std::cerr << "Finite Difference Jacobian (Joints)\n";
    std::cerr << finiteDifferenceJacobian.rightCols(
        derivativePoint.size() - (3 + iDynTree::sizeOfRotationParametrization(parametrization)))
              << "\n\n\n";
    ...
    for (size_t i = 0; i < derivativePoint.size(); ++i)
    {
        ...
        for (unsigned row = 0; row < (3 + iDynTree::sizeOfRotationParametrization(parametrization));
             ++row)
        {
