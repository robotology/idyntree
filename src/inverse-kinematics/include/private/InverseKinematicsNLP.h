...
class internal::kinematics::SparsityHelper
{
    ...
    bool addConstraintSparsityPatternTemplated(const MatrixType& newConstraint,
...
public:
     ...
     * @param fullMatrixStartingRowIndex starting row index of the non sparse matrix to be
     * considered. The size is automatically inferred by the constraint size
...
class internal::kinematics::InverseKinematicsNLP : public Ipopt::TNLP
{
    ...
    struct FrameInfo
    {
        ...
        iDynTree::MatrixFixSize<4, 3> quaternionDerivativeMap; /*!< map used during the derivative
                                                                  if the quaternion representation
                                                                  is used */
    };
    typedef std::map<int, FrameInfo> FrameInfoMap;

    InverseKinematicsData& m_data; /*!< Reference to the InverseKinematicsData object. Non
                                      IPOPT-specific data are saved and accessed in that object */

    // Buffers and variables used in the optimization
    iDynTree::MatrixFixSize<3, 4>
        quaternionDerivativeInverseMapBuffer; /*!< this is used to contain
                                                 the
                                                 quaternionDerivativeInverseMap,
                                                 computed once for each
                                                 optimization step */
    iDynTree::MatrixDynSize finalJacobianBuffer; /*!< Buffer to contain the Jacobian as modified to
                                                    handle quaternions */
    ...
    struct COMInfo
    {
        ...
        iDynTree::MatrixDynSize comJacobianAnalytical; /*!< 3 x ( 3 + sizeOfRotationParams + nDofs)
                                                          processed jacobian */
        iDynTree::MatrixDynSize projectedComJacobian; /*!< 2 x ( 3 + sizeOfRotationParams + nDofs)
                                                         processed jacobian */
    };
    ...
    iDynTree::Position optimizedBasePosition; /*!< Hold the base frame origin at an optimization
                                                 step */
    iDynTree::Vector4 optimizedBaseOrientation; /*!< Hold the base frame orientation at an
                                                   optimization step. Note that if orientation is
                                                   RPY, the last component should not be accessed */
    iDynTree::VectorDynSize jointsAtOptimisationStep; /*!< Hold the joints configuration at an
                                                         optimization step */
    ...
    {
        ComputeContraintJacobianOptionLinearPart = 1, /*!< Update the linear (position) part, i.e.
                                                         first 3 lines */
        ComputeContraintJacobianOptionAngularPart = 1 << 1, /*!< Update the angular part, i.e. last
                                                               3/4 lines depending on the
    ...
    void computeConstraintJacobian(
        const iDynTree::MatrixDynSize& transformJacobian,
    ...
    void
    computeConstraintJacobianRPY(const iDynTree::MatrixDynSize& transformJacobian,
    ...
    void
    computeConstraintJacobianCOMRPY(const iDynTree::MatrixDynSize& comJacobianBuffer,
     ...
     */
    void omegaToRPYParameters(const iDynTree::Vector3& rpyAngles, iDynTree::Matrix3x3& map);
     ...
     */
    void addSparsityInformationForConstraint(
...
#pragma mark - IpOpt methods
    ...
    virtual bool
    get_list_of_nonlinear_variables(Ipopt::Index num_nonlin_vars, Ipopt::Index* pos_nonlin_vars);
    ...
    void testDerivatives(const iDynTree::VectorDynSize& derivativePoint,
                         int frameIndex,
                         double epsilon,
                         double tolerance,
                         int parametrization);
