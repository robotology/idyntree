...
inline iDynTree::JointPosDoubleArray getRandomJointPositionsCloseTo(const iDynTree::Model& model, iDynTree::Span<const double> s, double maxDelta)
{
...
void simpleChainIK(int minNrOfJoints, int maxNrOfJoints, const iDynTree::InverseKinematicsRotationParametrization rotationParametrization)
{
    ...
    for (int i = minNrOfJoints; i <= maxNrOfJoints; i++)
    {
        ...
        // Add target
        ok = ik.addPositionTarget(targetFrame, MatrixView<const double>(kinDynDes.getWorldTransform(targetFrame).asHomogeneousTransform()));
        ASSERT_IS_TRUE(ok);
...
void simpleHumanoidWholeBodyIKConsistency(const iDynTree::InverseKinematicsRotationParametrization rotationParametrization)
{
    ...
    ok = ik.addFrameConstraint("l_foot", MatrixView<const double>(kinDynDes.getWorldTransform("l_foot").asHomogeneousTransform()));
    ASSERT_IS_TRUE(ok);
    ...
    ok = ik.addFrameConstraint("r_sole", MatrixView<const double>(kinDynDes.getWorldTransform("r_sole").asHomogeneousTransform()));
    ASSERT_IS_TRUE(ok);

    // The two cartesian targets should be reasonable values
    ik.setDefaultTargetResolutionMode(iDynTree::InverseKinematicsTreatTargetAsConstraintNone);
    ok = ik.addPositionTarget("l_elbow_1", MatrixView<const double>(kinDynDes.getWorldTransform("l_elbow_1").asHomogeneousTransform()));
    ASSERT_IS_TRUE(ok);
                                             ...
                                             size_t dofsToBeRemoved = 0)
{
    ...
    Eigen::Matrix4d basePosOptimized;
    Eigen::VectorXd sOptimized(ik.fullModel().getNrOfDOFs()), sOptimizedFromReduced(ik.fullModel().getNrOfDOFs());
    sOptimized.setZero();
...
void COMConvexHullConstraintWithSwitchingConstraints()
{
    ...
    ik.addFrameConstraint("r_sole", MatrixView<const double>(kinDynRef.getRelativeTransform("l_sole", "r_sole").asHomogeneousTransform()));
