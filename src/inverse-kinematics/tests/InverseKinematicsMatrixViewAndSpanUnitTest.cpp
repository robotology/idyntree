...
inline iDynTree::JointPosDoubleArray getRandomJointPositionsCloseTo(const iDynTree::Model& model,
                                                                    ...
                                                                    double maxDelta)
{
    ...
    for (iDynTree::JointIndex jntIdx = 0; jntIdx < model.getNrOfJoints(); jntIdx++)
    {
        ...
        } else
        {
            ...
            {
                int dofIndex = jntPtr->getDOFsOffset() + i;
                vec(jntPtr->getDOFsOffset() + i)
                    = iDynTree::getRandomDouble(s(dofIndex) - maxDelta, s(dofIndex) + maxDelta);
            }
        }
    }
...
void simpleChainIK(int minNrOfJoints,
                   int maxNrOfJoints,
                   const iDynTree::InverseKinematicsRotationParametrization rotationParametrization)
{
    ...
    for (int i = minNrOfJoints; i <= maxNrOfJoints; i++)
    {
        ...
        // Create also a KinDyn object to perform forward kinematics for the desired values and the
        // optimized ones
        ...
        Eigen::VectorXd s = toEigen(getRandomJointPositions(kinDynDes.model()));
        ok = kinDynDes.setJointPos(s);
        ASSERT_IS_TRUE(ok);

        // Add the fixed base constraint
        ok = ik.addFrameConstraint("link1", kinDynDes.getWorldTransform("link1"));
        // ok = ik.addFrameRotationConstraint("link1",
        // iDynTree::Transform::Identity().getRotation()); ok =
        // ik.addFramePositionConstraint("link1", iDynTree::Transform::Identity().getPosition());
        ASSERT_IS_TRUE(ok);

        // Add target
        ok = ik.addPositionTarget(targetFrame,
                                  MatrixView<const double>(kinDynDes.getWorldTransform(targetFrame)
        ...
        iDynTree::JointPosDoubleArray sInitial
            = getRandomJointPositionsCloseTo(ik.fullModel(), s, maxDelta);
        ik.setFullJointsInitialCondition(&(baseDes), &(sInitial));

        // Add a random desired position
        Eigen::VectorXd sDesired = toEigen(getRandomJointPositions(ik.fullModel()));
        ...
        // Get the solution
        Eigen::Matrix4d baseOpt;
        Eigen::VectorXd sOpt(ik.fullModel().getNrOfDOFs());
        ok = ik.getFullJointsSolution(baseOpt, sOpt);

        ASSERT_IS_TRUE(ok);

        Eigen::VectorXd dummyVel(6);
        dummyVel.setZero();
        Eigen::Vector3d dummyGrav;
        dummyGrav.setZero();
        Eigen::VectorXd dummyJointVel(ik.fullModel().getNrOfDOFs());
        dummyJointVel.setZero();
        ...
        ASSERT_EQUAL_TRANSFORM_TOL(kinDynOpt.getWorldTransform("link1"),
                                   kinDynDes.getWorldTransform("link1"),
                                   tolConstraints);

        // Check if the target is realized
        ASSERT_EQUAL_VECTOR_TOL(kinDynDes.getWorldTransform(targetFrame).getPosition(),
                                kinDynOpt.getWorldTransform(targetFrame).getPosition(),
                                tolTargets);
    }
}
    ...
    const iDynTree::InverseKinematicsRotationParametrization rotationParametrization)
{
    ...
    // Create a random vector of internal joint positions
    // used to get reasonable values for the constraints and the targets
    const auto s = getRandomJointPositions(kinDynDes.model());
    ok = kinDynDes.setJointPos(s);
    ASSERT_IS_TRUE(ok);
    ...
    ok = ik.addFrameConstraint("l_foot",
                               MatrixView<const double>(
                                   kinDynDes.getWorldTransform("l_foot").asHomogeneousTransform()));
    ASSERT_IS_TRUE(ok);

    std::cerr << "kinDynDes.getWorldTransform(l_foot) : "
              << kinDynDes.getWorldTransform("l_foot").toString() << std::endl;

    // The relative position of the r_sole should be the initial one
    ok = ik.addFrameConstraint("r_sole",
                               MatrixView<const double>(
                                   kinDynDes.getWorldTransform("r_sole").asHomogeneousTransform()));
    ASSERT_IS_TRUE(ok);

    // The two cartesian targets should be reasonable values
    ik.setDefaultTargetResolutionMode(iDynTree::InverseKinematicsTreatTargetAsConstraintNone);
    ok = ik.addPositionTarget("l_elbow_1",
                              MatrixView<const double>(kinDynDes.getWorldTransform("l_elbow_1")
                                                           .asHomogeneousTransform()));
    ASSERT_IS_TRUE(ok);

    // ok =
    // ik.addPositionTarget("r_elbow_1",kinDynDes.getRelativeTransform("l_sole","r_elbow_1").getPosition());
    // ASSERT_IS_TRUE(ok);
    ...
    Eigen::Matrix4d basePosOptimized;
    Eigen::VectorXd sOptimized(ik.fullModel().getNrOfDOFs());
    sOptimized.setZero();
    ...
    ASSERT_EQUAL_TRANSFORM_TOL(kinDynDes.getWorldTransform("l_foot"),
                               kinDynOpt.getWorldTransform("l_foot"),
                               tolConstraints);
    ASSERT_EQUAL_TRANSFORM_TOL(kinDynDes.getWorldTransform("r_sole"),
                               kinDynOpt.getWorldTransform("r_sole"),
                               tolConstraints);
    ASSERT_EQUAL_VECTOR_TOL(kinDynDes.getWorldTransform("l_elbow_1").getPosition(),
                            kinDynOpt.getWorldTransform("l_elbow_1").getPosition(),
                            tolTargets);
...
void simpleHumanoidWholeBodyIKCoMConsistency(
    const iDynTree::InverseKinematicsRotationParametrization rotationParametrization,
    ...
    size_t dofsToBeRemoved = 0)
{
    ...
    for (size_t index = 0; index < dofsToBeRemoved; ++index)
    {
        ...
        } while (std::find(toBeRemoved.begin(), toBeRemoved.end(), indexToBeRemoved)
                 != toBeRemoved.end());
        toBeRemoved.push_back(indexToBeRemoved);
    }
    std::sort(toBeRemoved.begin(), toBeRemoved.end());

    for (std::vector<int>::const_reverse_iterator rit = toBeRemoved.rbegin();
         rit != toBeRemoved.rend();
         ++rit)
    {
        std::cerr << "Removing DOF " << *(consideredJoints.begin() + (*rit)) << "(" << *rit << ")"
                  << std::endl;
    ...
    ok = kinDynDes.getCenterOfMassPosition(comDes);
    ASSERT_IS_TRUE(ok);

    ok = ik.setCOMTarget(comDes, 10);
    ASSERT_IS_TRUE(ok);
    ...
    sOptimized.setZero();
    sOptimizedFromReduced.setZero();
    Eigen::VectorXd sReducedOptimized(ik.reducedModel().getNrOfDOFs());
    sReducedOptimized.setZero();

    ok = ik.getFullJointsSolution(basePosOptimized, sOptimized);
    ASSERT_IS_TRUE(ok);

    ok = ik.getReducedSolution(basePosOptimized, sReducedOptimized);
    ASSERT_IS_TRUE(ok);
    ...
    ASSERT_EQUAL_VECTOR_TOL(kinDynDes.getCenterOfMassPosition(),
                            kinDynOpt.getCenterOfMassPosition(),
                            tolTargets);
...
// Check the consistency of a simple humanoid wholebody IK test case, setting a CoM target.
void simpleHumanoidWholeBodyIKCoMandChestConsistency(
    const iDynTree::InverseKinematicsRotationParametrization rotationParametrization,
    const iDynTree::InverseKinematicsTreatTargetAsConstraint targetResolutionMode)
{
    iDynTree::InverseKinematics ik;
    ...
    // The l_sole frame is our world absolute frame (i.e. {}^A H_{l_sole} = identity
    Eigen::Matrix4d lFootTransform;
    ok = kinDynDes.getWorldTransform("l_foot", lFootTransform);
    ASSERT_IS_TRUE(ok);

    ok = ik.addFrameConstraint("l_foot", lFootTransform);
    ASSERT_IS_TRUE(ok);
    ...
    // The relative position of the r_sole should be the initial one
    Eigen::Matrix4d rSoleTransform;
    ok = kinDynDes.getWorldTransform("r_sole", rSoleTransform);
    ASSERT_IS_TRUE(ok);

    ok = ik.addFrameConstraint("r_sole", rSoleTransform);
    ASSERT_IS_TRUE(ok);
    ...
    ok = ik.setCOMTarget(comDes, 1);
    ASSERT_IS_TRUE(ok);

    ik.setCOMAsConstraintTolerance(1e-8);

    Eigen::Matrix3d chestRotation = toEigen(kinDynDes.getWorldTransform("chest").getRotation());
    ok = ik.addRotationTarget("chest", chestRotation, 100);
    ASSERT_IS_TRUE(ok);
    ...
    ASSERT_EQUAL_VECTOR_TOL(kinDynDes.getCenterOfMassPosition(),
                            kinDynOpt.getCenterOfMassPosition(),
                            tolTargets);
    ASSERT_EQUAL_MATRIX_TOL(kinDynDes.getWorldTransform("chest").getRotation(),
                            kinDynOpt.getWorldTransform("chest").getRotation(),
                            tolTargets);
...
void COMConvexHullConstraintWithSwitchingConstraints()
{
    ...
    ik.addFrameConstraint("l_sole", identity);
    ik.addFrameConstraint("r_sole",
                          MatrixView<const double>(
                              kinDynRef.getRelativeTransform("l_sole", "r_sole")
                                  .asHomogeneousTransform()));

    // Add the COM convex hull constraint, where the polygon are just a long strip of 2 centimeters
    // at the origin of the frames The COM projection plane will always be the world XY plane, so
    // the world plane need to be always coincident with a support sole
    iDynTree::Direction xAxis(1.0, 0.0, 0.0);
    iDynTree::Direction yAxis(0.0, 1.0, 0.0);
    // The strips are long on the x direction so we don't need to care where the COM is actually in
    // the X direction
    double stripWidth = 0.02;
    ik.addCenterOfMassProjectionConstraint("l_sole",
                                           iDynTree::Polygon::XYRectangleFromOffsets(1,
                                                                                     1,
                                                                                     stripWidth / 2,
                                                                                     stripWidth
                                                                                         / 2),
    ...
    // Solve the problem in this case: as the com projection of all the joint set to 0 is in in
    // between the soles the inverse kinematics should give the joints all to zero as the solution
    clock_t tic = clock();
    ...
    ok = ik.solve();
    ASSERT_IS_TRUE(ok);
    std::cerr << "COMConvexHullConstraintWithSwitchingConstraints: IK with both constraints solved "
                 "in "
              << clockDurationInSeconds(clock() - tic) << "s" << std::endl;

    Eigen::Matrix4d baseSolution;
    Eigen::VectorXd jointPosSolution(ik.fullModel().getNrOfDOFs());
    ik.getFullJointsSolution(baseSolution, jointPosSolution);

    ASSERT_EQUAL_VECTOR_TOL(refereceJointPos, jointPosSolution, 1e-4);
    ...
    ok = ik.solve();
    std::cerr << "COMConvexHullConstraintWithSwitchingConstraints: IK with no constraints solved "
                 "in "
              << clockDurationInSeconds(clock() - tic) << "s" << std::endl;
    ASSERT_IS_TRUE(ok);
    ...
    ASSERT_EQUAL_VECTOR_TOL(refereceJointPos, jointPosSolution, 1e-3);

    // We only activate the left sole constraint, placing the "world" on the left_sole
    ok = ik.activateFrameConstraint("l_sole", identity);
    ASSERT_IS_TRUE(ok);
    ...
    // We solve the problem and verify that the y-component of com projected in the l_sole frame is
    // less than stripWidth/2
    tic = clock();
    ik.setFullJointsInitialCondition(&identityTransform, &refereceJointPos);
    ok = ik.solve();
    std::cerr << "COMConvexHullConstraintWithSwitchingConstraints: IK with l_sole constraint "
                 "solved in "
              << clockDurationInSeconds(clock() - tic) << "s" << std::endl;
    ASSERT_IS_TRUE(ok);
    ...
    Position com_l_sole
        = kinDynsol.getWorldTransform("l_sole").inverse() * (kinDynsol.getCenterOfMassPosition());
    ...
    ok = ik.activateFrameConstraint("r_sole", identity);
    ASSERT_IS_TRUE(ok);
    ...
    ik.setFullJointsInitialCondition(&identityTransform, &refereceJointPos);
    ok = ik.solve();
    std::cerr << "COMConvexHullConstraintWithSwitchingConstraints: IK with r_sole constraint "
                 "solved in "
              << clockDurationInSeconds(clock() - tic) << "s" << std::endl;
    ASSERT_IS_TRUE(ok);
    ...
    kinDynsol.setJointPos(jointPosSolution);
    Position com_r_sole
        = kinDynsol.getWorldTransform("r_sole").inverse() * (kinDynsol.getCenterOfMassPosition());
