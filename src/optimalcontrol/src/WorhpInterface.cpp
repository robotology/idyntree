...
class MatrixElement
{
    ...
    bool prioritySign() const
    {
        ...
        return (thisIndexDifference > 0) - (thisIndexDifference < 0); // +1 is the lower triangular
                                                                      // part, 0 the diagonal, -1
...
public:
    ...
    std::vector<size_t> constraintsJacNNZRows, constraintsJacNNZCols, hessianNNZRows,
        hessianNNZCols;
    size_t hessianLowerTriangularNonZeros;
    iDynTree::VectorDynSize initialPoint;
    iDynTree::VectorDynSize variablesLowerBounds, variablesUpperBounds, constraintsLowerBounds,
        constraintsUpperBounds;
    iDynTree::VectorDynSize variablesBuffer, constraintsEvaluationBuffer, costGradientBuffer,
        constraintsMultipliersBuffer;
    ...
    unsigned int previousNumberOfVariables, previousNumberOfConstraints, previousJacobianNonZeros,
        previousHessianNonZeros;
    ...
    bool possibleReOptimize(unsigned int newNumberOfVariables,
                            unsigned int newNumberOfconstraints,
                            unsigned int newJacobianNonZeros,
                            unsigned int newHessianNonZeros)
    {
...
bool WorhpInterface::solve()
{
    ...
    if (m_problem->info().hasSparseConstraintJacobian())
    {
                                                    ...
                                                    m_pimpl->constraintsJacNNZCols)))
        {
            reportError("WorhpInterface",
                        "solve",
                        "Error while retrieving constraints jacobian info.");
            m_pimpl->previouslySolved = false;
            return false;
        }
        ...
        if (m_pimpl->constraintsJacNNZCols.size() != m_pimpl->constraintsJacNNZRows.size())
        {
            reportError("WorhpInterface",
                        "solve",
                        "The vectors of nonzeros of the constraints jacobian have different "
                        "sizes.");
            m_pimpl->previouslySolved = false;
            return false;
        }
        ...
        for (size_t i = 0; i < m_pimpl->constraintsJacNNZRows.size(); ++i)
        {
            m_pimpl->orderedJacobianNonZeros[i].row
                = static_cast<unsigned int>(m_pimpl->constraintsJacNNZRows[i]);
            m_pimpl->orderedJacobianNonZeros[i].col
                = static_cast<unsigned int>(m_pimpl->constraintsJacNNZCols[i]);
        }
    ...
    if (m_problem->info().hasSparseHessian())
    {
        ...
        if (m_pimpl->hessianNNZRows.size() != m_pimpl->hessianNNZCols.size())
        {
            reportError("WorhpInterface",
                        "solve",
                        "The vectors of nonzeros of the hessian matrix have different sizes.");
            m_pimpl->previouslySolved = false;
            return false;
        }
        ...
        nonZerosHessian = static_cast<unsigned int>(m_pimpl->hessianLowerTriangularNonZeros)
                          + n; // add the diagonal elements

        m_pimpl->orderedHessianNonZeros.resize(m_pimpl->hessianNNZRows.size());

        for (size_t i = 0; i < m_pimpl->hessianNNZRows.size(); ++i)
        {
            m_pimpl->orderedHessianNonZeros[i].row
                = static_cast<unsigned int>(m_pimpl->hessianNNZRows[i]);
            m_pimpl->orderedHessianNonZeros[i].col
                = static_cast<unsigned int>(m_pimpl->hessianNNZCols[i]);
        }

        std::sort(m_pimpl->orderedHessianNonZeros.begin(),
                  m_pimpl->orderedHessianNonZeros.end(),
                  [](const MatrixElement& a, const MatrixElement& b) {
                      return a.lowerTriangularCompare(b);
                  });
    ...
    if (m_pimpl->possibleReOptimize(n, m, nonZerosJacobian, nonZerosHessian))
    {
        ...
        WorhpRestart(&(m_pimpl->worhp.opt),
                     &(m_pimpl->worhp.wsp),
                     &(m_pimpl->worhp.par),
                     &(m_pimpl->worhp.cnt));
    ...
    } else
    {
        ...
        } else
        {
            ...
            WorhpPreInit(&m_pimpl->worhp.opt,
                         &m_pimpl->worhp.wsp,
                         &m_pimpl->worhp.par,
                         &m_pimpl->worhp.cnt);
            ...
            for (auto boolParam = m_pimpl->boolParamsBackup.begin();
                 boolParam != m_pimpl->boolParamsBackup.end();
                 ++boolParam)
            {
                WorhpSetBoolParam(&(m_pimpl->worhp.par),
                                  boolParam->first.c_str(),
                                  boolParam->second);
            }

            for (auto intParam = m_pimpl->intParamsBackup.begin();
                 intParam != m_pimpl->intParamsBackup.end();
                 ++intParam)
            {
                WorhpSetIntParam(&(m_pimpl->worhp.par), intParam->first.c_str(), intParam->second);
            }

            for (auto doubleParam = m_pimpl->doubleParamsBackup.begin();
                 doubleParam != m_pimpl->doubleParamsBackup.end();
                 ++doubleParam)
            {
                WorhpSetDoubleParam(&(m_pimpl->worhp.par),
                                    doubleParam->first.c_str(),
                                    doubleParam->second);
            }
        }
    ...
    if (m_problem->getGuess(m_pimpl->initialPoint))
    {
        if (m_pimpl->initialPoint.size() != n)
        {
            reportError("WorhpInterface",
                        "solve",
                        "The specified guess size does not match the number of variables.");
            m_pimpl->previouslySolved = false;
            return false;
        }
        Eigen::Map<Eigen::VectorXd> initialPointMap(m_pimpl->worhp.opt.X, n);
    ...
    if (m_problem->getVariablesLowerBound(m_pimpl->variablesLowerBounds))
    {
        if (m_pimpl->variablesLowerBounds.size() != n)
        {
            reportError("WorhpInterface",
                        "solve",
                        "The size of the variables lowerbound does not match the number of "
                        "variables.");
            m_pimpl->previouslySolved = false;
            return false;
        }
    ...
    if (m_problem->getVariablesUpperBound(m_pimpl->variablesUpperBounds))
    {
        if (m_pimpl->variablesUpperBounds.size() != n)
        {
            reportError("WorhpInterface",
                        "solve",
                        "The size of the variables upperbound does not match the number of "
                        "variables.");
            m_pimpl->previouslySolved = false;
            return false;
        }
    ...
    if (!(m_problem->getConstraintsBounds(m_pimpl->constraintsLowerBounds,
                                          m_pimpl->constraintsUpperBounds)))
    {
    ...
    if (m_pimpl->constraintsLowerBounds.size() != m)
    {
        reportError("WorhpInterface",
                    "solve",
                    "Constraints bounds have size different from the number of constraints.");
        m_pimpl->previouslySolved = false;
        return false;
    }
    ...
    if (m_pimpl->worhp.wsp.DG.NeedStructure)
    {
        ...
        if (m_pimpl->sparseJacobian)
        {
            ...
            {
                m_pimpl->worhp.wsp.DG.row[el]
                    = static_cast<int>(m_pimpl->orderedJacobianNonZeros[el].row) + 1;
                m_pimpl->worhp.wsp.DG.col[el]
                    = static_cast<int>(m_pimpl->orderedJacobianNonZeros[el].col) + 1;
            }
        } else
        {
            size_t element = 0;
    ...
    if (m_pimpl->worhp.wsp.HM.NeedStructure)
    {
        ...
        if (m_pimpl->sparseHessian)
        {
            ...
            {
                m_pimpl->worhp.wsp.HM.row[el]
                    = static_cast<int>(m_pimpl->orderedHessianNonZeros[el].row) + 1;
                m_pimpl->worhp.wsp.HM.col[el]
                    = static_cast<int>(m_pimpl->orderedHessianNonZeros[el].col) + 1;
            }
     ...
     */
    while (m_pimpl->worhp.cnt.status < TerminateSuccess
           && m_pimpl->worhp.cnt.status > TerminateError)
    {
        /*
        ...
        {
            IterationOutput(&m_pimpl->worhp.opt,
                            &m_pimpl->worhp.wsp,
                            &m_pimpl->worhp.par,
                            &m_pimpl->worhp.cnt);
        ...
        if (GetUserAction(&m_pimpl->worhp.cnt, evalDF))
        {
            ...
            Eigen::Map<Eigen::VectorXd> costGradientMap(m_pimpl->worhp.wsp.DF.val, n); // dense
                                                                                       // vector
            costGradientMap
                = m_pimpl->worhp.wsp.ScaleObj * iDynTree::toEigen(m_pimpl->costGradientBuffer);
            ...
            if (m_pimpl->sparseJacobian)
            {
                for (size_t i = 0; i < m_pimpl->orderedJacobianNonZeros.size(); ++i)
                {
                    m_pimpl->worhp.wsp.DG.val[i]
                        = m_pimpl
                              ->constraintsJacobianBuffer(m_pimpl->orderedJacobianNonZeros[i].row,
                                                          m_pimpl->orderedJacobianNonZeros[i].col);
                }
            } else
            {
                ...
                for (unsigned int col = 0; col < n; ++col)
                {
                    for (unsigned int row = 0; row < m; ++row)
                    {
                        m_pimpl->worhp.wsp.DG.val[element]
                            = m_pimpl->constraintsJacobianBuffer(row, col);
                        element++;
                    }
                }
            }
        ...
        if (GetUserAction(&m_pimpl->worhp.cnt, evalHM))
        {
            ...
            if (!m_problem->evaluateConstraintsHessian(m_pimpl->constraintsMultipliersBuffer,
                                                       m_pimpl->constraintsHessianBuffer))
            {
    ...
    if (m_pimpl->worhp.cnt.status <= TerminateError)
    {
        reportError("WorhpInterface",
                    "solve",
                    "Failed to solve the problem. Here additional infos:");
        m_pimpl->previouslySolved = false;
        int printLevel;
        getWorhpParam("NLPprint", printLevel);
        setWorhpParam("NLPprint", 0);
        StatusMsg(&m_pimpl->worhp.opt,
                  &m_pimpl->worhp.wsp,
                  &m_pimpl->worhp.par,
                  &m_pimpl->worhp.cnt);
...
bool WorhpInterface::getPrimalVariables(iDynTree::VectorDynSize& primalVariables)
{
    if (!(m_pimpl->previouslySolved))
    {
        reportError("WorhpInterface",
                    "getPrimalVariables",
                    "The solve method was not called or it returned false.");
        return false;
    }
...
bool WorhpInterface::getDualVariables(iDynTree::VectorDynSize& constraintsMultipliers,
                                      iDynTree::VectorDynSize& lowerBoundsMultipliers,
                                      iDynTree::VectorDynSize& upperBoundsMultipliers)
{
    if (!(m_pimpl->previouslySolved))
    {
        reportError("WorhpInterface",
    ...
    Eigen::Map<const Eigen::VectorXd> constraintsMultipliersMap(m_pimpl->worhp.opt.Mu,
                                                                m_pimpl->worhp.opt.m);
    ...
    if (m_pimpl->variablesLowerBounds.size() == 0)
    {
        ...
        {
            Eigen::Map<const Eigen::VectorXd> variablesMultipliersMap(m_pimpl->worhp.opt.Lambda,
                                                                      m_pimpl->worhp.opt.n);
    ...
    } else
    {
        ...
        {
            lowerBoundDistance
                = std::fabs(m_pimpl->worhp.opt.X[i] - m_pimpl->variablesLowerBounds(i));
            upperBoundDistance
                = std::fabs(m_pimpl->worhp.opt.X[i] - m_pimpl->variablesUpperBounds(i));
...
bool WorhpInterface::setWorhpParam(const std::string& paramName, bool value)
{
    if (!WorhpSetBoolParam(&(m_pimpl->worhp.par), paramName.c_str(), value))
    {
        reportError("WorhpInterface",
                    "setWorhpParam",
                    ("Failed to set parameter " + paramName
                     + ". Either it was not found or the expected input value has wrong type.")
                        .c_str());
        return false;
    }
...
bool WorhpInterface::getWorhpParam(const std::string& paramName, bool& value)
{
    if (!WorhpGetBoolParam(&(m_pimpl->worhp.par), paramName.c_str(), &value))
    {
        reportError("WorhpInterface",
                    "getWorhpParam",
                    ("Failed to get parameter " + paramName
                     + ". Either it was not found or the expected output value has wrong type.")
                        .c_str());
        return false;
    }
    return true;
