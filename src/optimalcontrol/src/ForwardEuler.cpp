...
bool ForwardEuler::oneStepIntegration(double t0,
                                      double dT,
                                      const VectorDynSize& x0,
                                      VectorDynSize& x)
{
    if (!m_dynamicalSystem_ptr)
    {
        reportError(m_info.name().c_str(),
                    "oneStepIntegration",
                    "First you have to specify a dynamical system");
        return false;
    }
    ...
    if (!m_dynamicalSystem_ptr->dynamics(x0, t0, m_computationBuffer))
    {
        reportError(m_info.name().c_str(),
                    "oneStepIntegration",
                    "Error while evaluating the autonomous dynamics.");
        return false;
    }
...
ForwardEuler::ForwardEuler(const std::shared_ptr<DynamicalSystem> dynamicalSystem)
...
bool ForwardEuler::evaluateCollocationConstraint(double time,
...
bool ForwardEuler::evaluateCollocationConstraintJacobian(
    double time,
    const std::vector<VectorDynSize>& collocationPoints,
    const std::vector<VectorDynSize>& controlInputs,
    double dT,
...
bool ForwardEuler::getCollocationConstraintJacobianStateSparsity(
...
bool ForwardEuler::getCollocationConstraintJacobianControlSparsity(
...
bool ForwardEuler::evaluateCollocationConstraintSecondDerivatives(
    double time,
    const std::vector<VectorDynSize>& collocationPoints,
    const std::vector<VectorDynSize>& controlInputs,
    double dT,
...
bool ForwardEuler::getCollocationConstraintSecondDerivativeWRTStateSparsity(
...
bool ForwardEuler::getCollocationConstraintSecondDerivativeWRTControlSparsity(
...
bool ForwardEuler::getCollocationConstraintSecondDerivativeWRTStateControlSparsity(
