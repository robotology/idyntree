...
{
    Transform notRotated_H_rotated = ax.getRotationTransform(theta);
    Transform notRotated_H_rotated_validation = Transform(Rotation::Identity(), ax.getOrigin())
...
Matrix6x6 RotAdjWrenchTransformNumericalDerivative(const Axis& ax, double theta, double step)
{
    ...
    Matrix6x6 perturbatedUpper
        = ax.getRotationTransform(theta + step / 2).asAdjointTransformWrench();
    Matrix6x6 perturbatedLower
        = ax.getRotationTransform(theta - step / 2).asAdjointTransformWrench();
...
void validateRotationTransformDerivative(const Axis& ax, double theta)
{
    ...
    Matrix4x4 homTransformDerivNum
        = RotHomTransformNumericalDerivative(ax, theta, numericalDerivStep);

    Matrix6x6 adjTransformDerivAn = analytic.asAdjointTransformDerivative(trans);
    Matrix6x6 adjTransformDerivNum
        = RotAdjTransformNumericalDerivative(ax, theta, numericalDerivStep);

    Matrix6x6 adjWrenchTransformDerivAn = analytic.asAdjointTransformWrenchDerivative(trans);
    Matrix6x6 adjWrenchTransformDerivNum
        = RotAdjWrenchTransformNumericalDerivative(ax, theta, numericalDerivStep);
