...
SpatialInertia RigidBodyInertiaNonLinearParametrization::toRigidBodyInertia() const
{
    ...
    principalMomentOfInertiaAtCOM(0)
        = this->centralSecondMomentOfMass(1) + this->centralSecondMomentOfMass(2);
    principalMomentOfInertiaAtCOM(1)
        = this->centralSecondMomentOfMass(0) + this->centralSecondMomentOfMass(2);
    principalMomentOfInertiaAtCOM(2)
        = this->centralSecondMomentOfMass(0) + this->centralSecondMomentOfMass(1);

    toEigen(rotInertia) = toEigen(this->link_R_centroidal) * diag(principalMomentOfInertiaAtCOM)
...
Vector16 RigidBodyInertiaNonLinearParametrization::asVectorWithRotationAsVec() const
{
    ...
    Map<Matrix<double, 9, 1>>(vectorization.data() + 4)
        = vecColMajor(toEigen(this->link_R_centroidal));
...
void RigidBodyInertiaNonLinearParametrization::fromVectorWithRotationAsVec(
    const Vector16& vectorization)
{
    using namespace Eigen;
    ...
    toEigen(this->link_R_centroidal)
        = unvecColMajor(Map<const Matrix<double, 9, 1>>(vectorization.data() + 4));
...
{
    return (this->mass > 0) && (this->centralSecondMomentOfMass(0) >= 0.0)
...
Matrix10x16 RigidBodyInertiaNonLinearParametrization::getGradientWithRotationAsVec() const
{
    ...
    regrWrtCOM << 0, 2 * m * cy, 2 * m * cz, -m * cy, -m * cx, 0, -m * cz, 0, -m * cx, 2 * m * cx,
        0, 2 * m * cz, 0, -m * cz, -m * cy, 2 * m * cx, 2 * m * cy, 0;
