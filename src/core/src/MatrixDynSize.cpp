...
MatrixDynSize::MatrixDynSize(std::size_t _rows, std::size_t _cols)
    : m_rows(_rows)
    , m_cols(_cols)
{
...
MatrixDynSize::MatrixDynSize(MatrixView<const double> other)
    : m_rows(other.rows())
    , m_cols(other.cols())
{
    if (this->m_rows * this->m_cols == 0)
...
void MatrixDynSize::changeCapacityAndCopyData(const std::size_t _newCapacity)
{
    ...
    if (m_data)
    {
        ...
        std::memcpy(newBuffer,
                    m_data,
                    std::min(this->m_rows * this->m_cols, _newCapacity) * sizeof(double));
    }
...
{
    if ((_newRows == this->rows()) && (_newCols == this->cols()))
    {
        return;
    }
