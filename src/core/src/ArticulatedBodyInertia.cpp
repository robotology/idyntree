...
mySkewABI(const Eigen::MatrixBase<Derived>& vec)
{
...
ArticulatedBodyInertia::ArticulatedBodyInertia(const ArticulatedBodyInertia& other)
    : linearLinear(other.linearLinear)
    , linearAngular(other.linearAngular)
    , angularAngular(other.angularAngular)
{
}
...
Wrench ArticulatedBodyInertia::operator*(const SpatialAcc& other) const
{
    ...
    toEigen(ret.getAngularVec3())
        = toEigen(this->linearAngular).transpose() * toEigen(other.getLinearVec3())
...
ArticulatedBodyInertia::ABADyadHelper(const SpatialForceVector& U, const double d)
{
...
ArticulatedBodyInertia ArticulatedBodyInertia::ABADyadHelperLin(const SpatialForceVector& U,
                                                                const double inv_d,
                                                                const SpatialForceVector& /*dU*/,
                                                                const double d_inv_d)
{
    ArticulatedBodyInertia ret;
