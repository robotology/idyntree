...
{
/**
 * @brief Struct containing the options for geodesicL2MeanRotation and
 * geodesicL2WeightedMeanRotation.
 */
struct GeodesicL2MeanOptions
{
    double tolerance{1e-5}; /** Tolerance for terminating the inner refinement loop of the mean
                               rotation. **/
 ...
 *
 * It implements the angular distance presented in Sec. 4 of "Rotation Averaging" (available at
 * http://users.cecs.anu.edu.au/~hongdong/rotationaveraging.pdf), in particular \f$d =
 * ||log(R_1^\top R_2)||^2 \f$.
 ...
 *
 * It implements Algorithm 1 in Sec. 5.3 of "Rotation Averaging" (available at
 ...
 *
 * It implements Algorithm 1 in Sec. 5.3 of "Rotation Averaging" (available at
 * http://users.cecs.anu.edu.au/~hongdong/rotationaveraging.pdf), with a small modification to take
 * into accounts weights different from 1.
 *
 * @param inputRotations The rotations to average.
 * @param weights The weights for each rotation. If this vector is null assumes that each weight
 * is 1.0 (equivalent to geodesicL2MeanRotation)
