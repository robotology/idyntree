...
public:
    ...
    IDYNTREE_CONSTEXPR const reference getVal(index_type idx) const { return this->operator[](idx);}
    IDYNTREE_CONSTEXPR bool setVal(index_type idx, const reference val)
    {
        assert(idx >= 0 && idx < storage_.size());
        data()[idx] = val;
        return true;
