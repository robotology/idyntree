...
typedef const Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
    iDynTreeEigenConstMatrix;
...
#if __cplusplus > 199711L
template <typename> struct is_sparsematrix : std::false_type
{
};
...
{
    return Eigen::Map<
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>(mat.data(),
                                                                                mat.rows(),
                                                                                mat.cols());
}
...
toEigen(const MatrixView<const double>& mat)
{
    ...
    // The inner stride is the pointer increment between two consecutive entries within a given row
    // of a row-major matrix. The outer stride is the pointer increment between two consecutive rows
    ...
    const std::ptrdiff_t innerStride
        = (mat.storageOrder() == MatrixStorageOrdering::ColumnMajor) ? mat.rows() : 1;
    const std::ptrdiff_t outerStride
        = (mat.storageOrder() == MatrixStorageOrdering::ColumnMajor) ? 1 : mat.cols();
...
inline Eigen::Map<const Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>
toEigen(const MatrixDynSize& mat)
{
    return Eigen::Map<
        const Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>(mat.data(),
                                                                                      mat.rows(),
                                                                                      mat.cols());
}
...
template <unsigned int VecSize>
inline Eigen::Map<const Eigen::Matrix<double, VecSize, 1>>
toEigen(const VectorFixSize<VecSize>& vec)
{
    return Eigen::Map<const Eigen::Matrix<double, VecSize, 1>>(vec.data());
}

template <unsigned int nRows, unsigned int nCols>
inline Eigen::Map<Eigen::Matrix<double, nRows, nCols, Eigen::RowMajor>>
toEigen(MatrixFixSize<nRows, nCols>& mat)
{
...
template <unsigned int nRows, unsigned int nCols>
inline Eigen::Map<const Eigen::Matrix<double, nRows, nCols, Eigen::RowMajor>>
toEigen(const MatrixFixSize<nRows, nCols>& mat)
{
...
skew(const Eigen::MatrixBase<Derived>& vec)
{
    ...
    return (Eigen::Matrix<typename Derived::Scalar, 3, 3, Eigen::RowMajor>() << 0.0,
            -vec[2],
            vec[1],
            vec[2],
            0.0,
...
inline Eigen::Matrix<typename Derived::Scalar, 3, 1> unskew(const Eigen::MatrixBase<Derived>& mat)
{
    ...
    return (Eigen::Matrix<typename Derived::Scalar, 3, 1>() << mat(2, 1), mat(0, 2), mat(1, 0))
        .finished();
}
 ...
 */
template <class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 4, 4, Eigen::RowMajor>
wedge6dTo4x4d(const Eigen::MatrixBase<Derived>& vec)
{
...
vee4x4dTo6d(const Eigen::MatrixBase<Derived>& mat)
{
...
{
#if __cplusplus > 199711L
    static_assert(!iDynTree::is_sparsematrix<iDynTreeMatrixType>::value,
                  "You cannot set a subMatrix on a SparseMatrix.");
#endif

    toEigen(mat).block(rowRange.offset, colRange.offset, rowRange.size, colRange.size)
        = toEigen(subMat);
    return;
}
...
#if __cplusplus > 199711L
    static_assert(!iDynTree::is_sparsematrix<iDynTreeMatrixType>::value,
                  "You cannot set a setSubMatrixToIdentity on a SparseMatrix.");
#endif
...
#if __cplusplus > 199711L
    static_assert(!iDynTree::is_sparsematrix<iDynTreeMatrixType>::value,
                  "You cannot set a setSubMatrixToMinusIdentity on a SparseMatrix.");
#endif
    assert(rowRange.size == colRange.size);
    for (int i = 0; i < rowRange.size; i++)
    {
...
inline void
setSubVector(VectorDynSize& vec, const IndexRange range, const VectorFixSize<size>& subVec)
{
    toEigen(vec).segment(range.offset, range.size) = toEigen(subVec);
    return;
}

inline void setSubVector(VectorDynSize& vec, const IndexRange range, double subVec)
{
    assert(range.size == 1);
...
inline void
setSubVector(VectorDynSize& vec, const IndexRange range, const SpatialMotionVector& twist)
{
    assert(range.size == 6);
...
inline void
setSubVector(VectorDynSize& vec, const IndexRange range, const SpatialForceVector& wrench)
{
    assert(range.size == 6);
...
template <typename T>
inline void setSubVector(VectorDynSize& vec, const IndexRange range, const T& subVec)
{
