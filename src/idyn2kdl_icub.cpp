           ...
           bool debug)
{
    ...
    if( ft_foot )
    {
        ...
        status_ok = idynSensorChain2kdlChain(*(icub_idyn_feetv2.lowerTorso->left),*(icub_idyn_feetv2.lowerTorso->leftSensor),old_llV2,ll_links,ll_joints,"l_sole");
        if(!status_ok) return false;

        //Creating right leg
        KDL::Chain rlV2, old_rlV2;
        status_ok = idynSensorChain2kdlChain(*(icub_idyn_feetv2.lowerTorso->right),*(icub_idyn_feetv2.lowerTorso->rightSensor),old_rlV2,rl_links,rl_joints,"r_sole");
        if(!status_ok) return false;
        ...
        rl.addSegment(r_upper_foot); rl.addSegment(r_foot_new); rl.addSegment(no_ft_rlV2.getSegment(7));

        ll.addSegment(l_upper_foot); ll.addSegment(l_foot_new); ll.addSegment(no_ft_llV2.getSegment(7));
    }
