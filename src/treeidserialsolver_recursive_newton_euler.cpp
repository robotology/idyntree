...
namespace KDL{
namespace CoDyCo{    
    TreeIdSerialSolver_RNE::TreeIdSerialSolver_RNE(const Tree& tree, const TreeSerialization & serialization) 
    : TreeSerialSolver(tree,serialization)
    {
        //duplicate TreeSerialSolver inizialitation !!, should fix 
        TreeIdSerialSolver_RNE(tree,Vector::Zero(),serialization);
    }
    
    TreeIdSerialSolver_RNE::TreeIdSerialSolver_RNE(const Tree& tree, Vector grav, const TreeSerialization & serialization)
    : TreeSerialSolver(tree,serialization)
    {
    ...
    int TreeIdSerialSolver_RNE::CartToJnt(const KDL::JntArray &q, const KDL::JntArray &q_dot, const KDL::JntArray &q_dotdot, const Wrenches& f_ext,JntArray &torques)
    {
        Wrench dummy;
    ...
    int TreeIdSerialSolver_RNE::CartToJnt(const KDL::JntArray &q, const KDL::JntArray &q_dot, const KDL::JntArray &q_dotdot, const Twist& base_velocity, const Twist& base_acceleration, const Wrenches& f_ext,JntArray &torques, Wrench& base_force)
    {
        ...
        for( l = 0; l < (int)recursion_order.size(); l++ ) {
            ...
            #ifndef NDEBUG
            //std::cout << "v[ " << index2segment[curr_index]->first << " ] = " << ev << std::endl;
            //std::cout << "a[ " << index2segment[curr_index]->first << " ] = " << ea << std::endl;
            #endif 
