                             ...
                             FreeFloatingMassMatrixDerivative& massMatrixDerivative)
{
    ...
    Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>
        massMatrixEigen(massMatrix.data(), massMatrix.rows(), massMatrix.cols());
    Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>
        massMatrixDerivativeEigen(massMatrixDerivative.data(), massMatrixDerivative.rows(), massMatrixDerivative.cols());
    ...
    for(int traversalEl = traversal.getNrOfVisitedLinks() - 1; traversalEl >= 0; traversalEl--){
        ...
        { // the visited link is NOT the base link
            ...
            if (toParentJoint->getNrOfDOFs() == 0)
            {
                ...
                LinkIndex parentLinkIndex = traversal.getParentLinkFromLinkIndex(visitedLinkIndex)->getIndex();
                Transform link_X_parentLink = linkPos(visitedLinkIndex).inverse() * linkPos(parentLinkIndex);
    ...
    for(int traversalEl = traversal.getNrOfVisitedLinks() - 1; traversalEl >= 0; traversalEl--){
        ...
        if (visitedLinkIndex){
            ...
            { // is NOT a fixed joint
                ...
                for (unsigned int dofIdx = 0; dofIdx < nrOfDOFs; dofIdx++)
                {
                    ...
                    toEigen(Sdot_visitedDof) = toEigen(linkVels(visitedLinkIndex).asCrossProductMatrix()) * toEigen(S_visitedDof);

                    // compute F1, F2, F3
                    Vector6 F1, F2, F3;
                    toEigen(F1) = toEigen(linkCRBIs(visitedLinkIndex).asMatrix()) * toEigen(Sdot_visitedDof)
                                        + toEigen(B[visitedLinkIndex]) * toEigen(S_visitedDof);
                    toEigen(F2) = toEigen(linkCRBIs(visitedLinkIndex).asMatrix()) * toEigen(S_visitedDof);
                         ...
                         otherDofIdx++)
                    {
                        ...
                        toEigen(Sdot_otherDof) = toEigen(linkVels(visitedLinkIndex).asCrossProductMatrix()) * toEigen(S_otherDof);

                        // Compute F for the other DOF as well
                        Vector6 F1_other, F2_other, F3_other;
                        toEigen(F1_other) = toEigen(linkCRBIs(visitedLinkIndex).asMatrix()) * toEigen(Sdot_otherDof)
                                              + toEigen(B[visitedLinkIndex]) * toEigen(S_otherDof);
                        toEigen(F2_other) = toEigen(linkCRBIs(visitedLinkIndex).asMatrix()) * toEigen(S_otherDof);
                    ...
                    {
                        LinkConstPtr ancestorParentLink = traversal.getParentLinkFromLinkIndex(ancestorLinkIndex);
                        LinkIndex ancestorParentIndex = ancestorParentLink->getIndex();

                        Transform ancestorLink_X_parentLink = linkPos(ancestorLinkIndex).inverse() * linkPos(ancestorParentIndex);
                        ...
                        IJointConstPtr toGrandParentJoint = traversal.getParentJointFromLinkIndex(ancestorParentIndex);
                        if (toGrandParentJoint->getNrOfDOFs() == 0){
                            ...
                            for (unsigned int ancestorDofIdx = 0; ancestorDofIdx < ancestorNrOfDOFs; ancestorDofIdx++)
                            {
                                ...
                                toEigen(S_ancestorParentDof) = toEigen(toGrandParentJoint->getMotionSubspaceVector(ancestorDofIdx, ancestorParentIndex,
                                                                                         traversal.getParentLinkFromLinkIndex(ancestorParentIndex)->getIndex()));
                                toEigen(Sdot_ancestorParentDof) = toEigen(linkVels(ancestorParentIndex).asCrossProductMatrix()) * toEigen(S_ancestorParentDof);
                    ...
                    Transform otherLink_X_parentLink = linkPos(ancestorLinkIndex).inverse() * linkPos(ancestorParentIndex);
                    ...
                    coriolisMatrixEigen.block<1, 6>(6+dofIndex, 0) = toEigen(F3_ancestor).transpose();
                    ...
                    massMatrixDerivativeEigen.block<6, 1>(0, 6 + dofIndex) = toEigen(F1_ancestor) + toEigen(F3_ancestor);
                    massMatrixDerivativeEigen.block<1, 6>(6 + dofIndex, 0) = (toEigen(F1_ancestor) + toEigen(F3_ancestor)).transpose();
                } // end of DOF loop
        ...
        else
        {
            ...
            massMatrixDerivativeEigen.block<6, 6>(0, 0) = toEigen(B[visitedLinkIndex]) + toEigen(B[visitedLinkIndex]).transpose();
        }
