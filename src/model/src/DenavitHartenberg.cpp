...
bool DHChain::fromModel(const Model& model, std::string baseFrame, std::string eeFrame)
{
...
Transform TransformFromDHCraig1989(double a, double alpha, double d, double theta)
{
    ...
    sa = sin(alpha);
    ca = cos(alpha);
    return Transform(Rotation(ct, -st, 0, st * ca, ct * ca, -sa, st * sa, ct * sa, ca),
...
// returns Denavit-Hartenberg parameters (Non-Modified DH)
{
    ...
    sa = sin(alpha);
    ca = cos(alpha);
    return Transform(Rotation(ct, -st * ca, st * sa, st, ct * ca, -ct * sa, 0, sa, ca),
                            ...
                            double tol = 1e-6)
{
    ...
    bool areAxesNotParallel
        = closestPoints(line_A, line_B, closest_point_line_A, closest_point_line_B);

    if (areAxesNotParallel)
    {
        bool arePointCoincident
            = ((toEigen(closest_point_line_A) - toEigen(closest_point_line_B)).norm() < tol);
    ...
    } else
    {
        ...
        bool areOriginCoincident
            = ((toEigen(line_A.getOrigin()) - toEigen(line_B.getOrigin())).norm() < tol);
        ...
        toEigen(originDiffDirection)
            = (toEigen(line_A.getOrigin()) - toEigen(line_B.getOrigin())).normalized();
 ...
 *
 * The _hint variable are necessary when the DH convention allow freedom of choice in choosing
 * dh_direction_axis_x_n or dh_origin_n, to preserve injectivity.
 *
                 ...
                 int verbose = 0)
{
    ...
    //          or in the direction normal to the z_{i-1} - z_{i} plane if z_{i-1} and z_i
    //          intersect.

    // While the book distingush just two cases for STEP 4 , we need to actually distinguish three
    // different cases :
    // * z_{i-1} and z_{i} are not incident : in this case the direction of the common normal is
    // always defined,
    //   and in particular we compute it as the difference between the two origins, projected on the
    //   spaces orthogonal to z_i.
    // * z_{i-1} and z_{i} are incident, but not coincident: then the two axis define a plane with a
    // unique axis
    // * z_{i-1} and z_{i} are coincident (i.e. incident and parallel): in this case there are
    // infinite normals,
    //   and so we choose x_i using the hint provided (tipically the x_i used in the previous
    //   representation).
    if (!zAxes_incident)
    {
        // not incident
        ...
        Eigen::Vector3d x_i_candidate_eig
            = origin_diff
              - origin_diff.dot(toEigen(zAxis_i_minus_1.getDirection()))
                    * toEigen(zAxis_i_minus_1.getDirection());
    ...
    } else
    {
        if (!zAxes_coincident)
        {
            ...
            Eigen::Vector3d x_i_candidate_eig
                = toEigen(zAxis_i_minus_1.getDirection()).cross(toEigen(zAxis_i.getDirection()));
            x_i_candidate_eig.normalize();

            // Check direction, use hint
            // The positive direction of the axis_x_n is arbitrary, however for dealing with limit
            // case (link where
            // only alpha is different from zero)
            double dh_direction_axis_x_n_sign
                = x_i_candidate_eig.dot(toEigen(xAxis_n_direction_hint));
            ...
            std::cerr << "ExtractDHChainFromModel : DH representation is not unique, made an "
                         "arbitrary choice for x_i sign\n";
        } else
        {
            // coincident
            // if the two axis are coincident, the direction of dh_direction_axis_x_n is totally
            // arbitrary as long as it is perpendicular to zAxis. We will take then the hint
            // provided by : direction_axis_x_n_hint to get the dh_direction_axis_x_n, we will
            // project direction_axis_x_n_hint onto the plane perpendicular to axis_z_n_minus_1 ==
            // axis_z_n, and will normalize the resulting vector if dh_direction_axis_x_n is
            // parallel to the two axis, we just apply the same procedure to (1,0,0), (0,1,0) and
            // (0,0,1) (in the base frame) in this order. At least two of this three vectors will
            // not be parallel with the axis of the joint, and so they can be used as hints for the
            // direction of
            ...
            Eigen::Vector3d x_i_candidate_eig
                = toEigen(xDirectionHintNotParallelToZ)
            ...
            std::cerr << "ExtractDHChainFromModel : DH representation is not unique, made an "
                         "arbitrary choice for x_i direction\n";
        }
    }
    ...
    dhParams.A = -(toEigen(x_i_z_i_minus_1_intersection_B) - toEigen(origin_i))
                      .dot(toEigen(xAxis_i.getDirection()));

    // calculation of d_i
    // distance along z_{i-1} from O_{i-1} to the intersection of the x_i and z_{i-1} axes
    dhParams.D = (toEigen(x_i_z_i_minus_1_intersection_A) - toEigen(origin_i_minus_1))
                     .dot(toEigen(zAxis_i_minus_1.getDirection()));

    // calculation of alpha_i
    // angle between z_{i-1} and z_i measured about x_i
    double cos_alpha_i
        = toEigen(zAxis_i_minus_1.getDirection()).dot(toEigen(zAxis_i.getDirection()));
    ...
    double cos_theta_i
        = toEigen(xAxis_i_minus_1.getDirection()).dot(toEigen(xAxis_i.getDirection()));
    double sin_theta_i
        = (toEigen(xAxis_i_minus_1.getDirection()).cross(toEigen(xAxis_i.getDirection())))
              .dot(toEigen(zAxis_i_minus_1.getDirection()));
                             ...
                             double tolerance)
{
    ...
    size_t nrOfDofs = 0;
    for (LinkIndex visitedLinkIdx = linkOfEEFrameIdx; visitedLinkIdx != linkOfBaseFrameIdx;
         visitedLinkIdx = traversal.getParentLinkFromLinkIndex(visitedLinkIdx)->getIndex())
    {
        ...
        bool isRevoluteJoint = (dynamic_cast<const RevoluteJoint*>(joint) != 0);
        bool isFixedJoint = (dynamic_cast<const FixedJoint*>(joint) != 0);

        if (!isFixedJoint && !isRevoluteJoint)
        {
            std::string err = "Joint  " + model.getJointName(joint->getIndex())
                              + " is not revolute neither fixed, but the DH converter only support "
                                "this two joints.";
            reportError("", "ExtractDHChainFromModel", err.c_str());
        }
         ...
         visitedLinkIdx = traversal.getParentLinkFromLinkIndex(visitedLinkIdx)->getIndex())
    {
        ...
        if (isRevoluteJoint)
        {
            ...
            zAxes[counter - 1]
                = baseFrame_X_link(visitedLinkIdx) * revJoint->getAxis(visitedLinkIdx);
    ...
    Transform base_H_endEffector = baseFrame_X_link(linkOfEEFrameIdx)
                                   * model.getFrameTransform(model.getFrameIndex(eeFrame));
    ...
    } else
    {
        ...
        Eigen::Vector3d nonNormalizedRotAxis
            = toEigen(Direction(0, 0, 1.0)).cross(toEigen(zAxes[0].getDirection()));
        toEigen(rotAxis) = nonNormalizedRotAxis;
        rotAxis.Normalize();
        double rot_angle
            = atan2(toEigen(rotAxis).dot(nonNormalizedRotAxis),
                    toEigen(Direction(0, 0, 1.0)).dot(toEigen(zAxes[0].getDirection())));
    ...
    // We must select the origin, the x and y axis of dh_0 frame so we can initialize the recursive
    // procedure that identifies the origin, x and y of each dh frame (the z axis are already
    // determined by the axes directions)
    ...
    for (size_t i = 0; i < nrOfDofs; i++)
    {
        ...
        if (!ok)
        {
            reportError("",
                        ...
                        "Error in extracing dh parameters for one link of the chain");
        }
    ...
    Transform base_H_N;
    ok = transformFromAxes(xAxes[nrOfDofs], yAxes[nrOfDofs], zAxes[nrOfDofs], base_H_N);
    globalOk = globalOk && ok;
...
bool CreateModelFromDHChain(const DHChain& inputChain, Model& outputModel)
{
    ...
    for (int i = 0; i < inputChain.getNrOfDOFs(); i++)
    {
        ...
        Transform dhTransform = TransformFromDH(inputChain(i).A,
                                                inputChain(i).Alpha,
                                                inputChain(i).D,
                                                inputChain(i).Offset);
    ...
    outputModel.addAdditionalFrameToLink("link" + intToString(inputChain.getNrOfDOFs()),
                                         "distalFrame",
                                         inputChain.getHN());
