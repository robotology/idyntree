...
bool SixAxisForceTorqueSensor::setFirstLinkSensorTransform(
    const LinkIndex link_index, const iDynTree::Transform& link_H_sensor) const
{
...
bool SixAxisForceTorqueSensor::setSecondLinkSensorTransform(
    const LinkIndex link_index, const iDynTree::Transform& link_H_sensor) const
{
...
bool SixAxisForceTorqueSensor::isValid() const
{
    ...
    if (this->pimpl->link1 < 0 && this->pimpl->link2 < 0 && this->pimpl->appliedWrenchLink < 0)
    {
        // Return false because the links is not appropriately setted
        return false;
    }

    if (this->pimpl->link1 != this->pimpl->appliedWrenchLink
...
bool SixAxisForceTorqueSensor::updateIndices(const Model& model)
{
    ...
    iDynTree::JointIndex parentJointNewIndex
        = model.getJointIndex(this->pimpl->parent_junction_name);
...
{
    return (this->pimpl->link1 == link_index || this->pimpl->link2 == link_index);
}

bool SixAxisForceTorqueSensor::getLinkSensorTransform(const LinkIndex link_index,
...
bool SixAxisForceTorqueSensor::getWrenchAppliedOnLinkMatrix(
    const LinkIndex link_index, Matrix6x6& wrench_applied_on_link_matrix) const
{
    assert(this->isValid());
    ...
    if (link_index == this->pimpl->link1)
    {
        ...
        if (this->getAppliedWrenchLink() != link_index)
        {
            toEigen(wrench_applied_on_link_matrix)
                = -toEigen(link_H_sensor.asAdjointTransformWrench());
        } else
        {
            toEigen(wrench_applied_on_link_matrix)
                = toEigen(link_H_sensor.asAdjointTransformWrench());
...
bool SixAxisForceTorqueSensor::getWrenchAppliedOnLinkInverseMatrix(
    const LinkIndex link_index, Matrix6x6& wrench_applied_on_link_inverse_matrix) const
{
    assert(this->isValid());
    ...
    if (link_index == this->pimpl->link1)
    {
        ...
        if (this->getAppliedWrenchLink() != link_index)
        {
            toEigen(wrench_applied_on_link_inverse_matrix)
                = -toEigen(sensor_H_link.asAdjointTransformWrench());
        } else
        {
            toEigen(wrench_applied_on_link_inverse_matrix)
                = toEigen(sensor_H_link.asAdjointTransformWrench());
...
Wrench SixAxisForceTorqueSensor::predictMeasurement(const Traversal& traversal,
                                                    const LinkInternalWrenches& intWrenches)
{
    ...
    assert(this->isValid());
    assert(this->getFirstLinkIndex() >= 0
           && this->getFirstLinkIndex() < static_cast<LinkIndex>(traversal.getNrOfVisitedLinks()));
    assert(this->getSecondLinkIndex() >= 0
           && this->getSecondLinkIndex() < static_cast<LinkIndex>(traversal.getNrOfVisitedLinks()));
    ...
    if (traversal.getParentLinkFromLinkIndex(this->getFirstLinkIndex()) != 0
        && traversal.getParentLinkFromLinkIndex(this->getFirstLinkIndex())->getIndex()
               == this->getSecondLinkIndex())
    {
        child_link = this->getFirstLinkIndex();
        parent_link = this->getSecondLinkIndex();
    } else
    {
        assert(traversal.getParentLinkFromLinkIndex(this->getSecondLinkIndex())->getIndex()
               == this->getFirstLinkIndex());
...
std::string SixAxisForceTorqueSensor::toString(const Model& /*model*/) const
{
    ...
    ss << " is attached to joint " << this->getParentJoint() << " ( " << this->getParentJointIndex()
       << " ) " << std::endl;
    ss << " that connects  " << this->getFirstLinkName() << " ( " << this->getFirstLinkIndex()
       << " ) and " << this->getSecondLinkName() << " ( " << this->getSecondLinkIndex()
       << std::endl;
