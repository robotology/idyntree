...
void checkCOPcomputation(const ThreeAxisForceTorqueContactSensor& sensor)
{
    ...
    Position copIn3D
        = sensor.computeCenterOfPressureFromLoadCellMeasurements(arbitraryLoadCellReadings);
    Vector2 cop;
    cop(0) = copIn3D(0);
    cop(1) = copIn3D(1);
    Vector2 copCheck;
    Vector3 measuredFT
        = sensor.computeThreeAxisForceTorqueFromLoadCellMeasurements(arbitraryLoadCellReadings);
