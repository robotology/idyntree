...
{
    // Extract joint wrenches
    iDynTree::LinkInternalWrenches intWrenches(berdyHelper.model());

    const iDynTree::Model& model = berdyHelper.model();

    for (iDynTree::JointIndex jntIdx = 0; jntIdx < static_cast<iDynTree::JointIndex>(model.getNrOfJoints()); ++jntIdx)
    {
        ...
        LinkIndex childLinkIndex  = berdyHelper.dynamicTraversal().getChildLinkIndexFromJointIndex(model, jntIdx);
        LinkIndex parentLinkIndex = berdyHelper.dynamicTraversal().getParentLinkIndexFromJointIndex(model, jntIdx);

        iDynTree::IndexRange range = berdyHelper.getRangeJointVariable(JOINT_WRENCH, jntIdx);
        iDynTree::LinearForceVector3   force(estimatedDynamicVariables.data() + range.offset, 3);
        iDynTree::AngularMotionVector3 torque(estimatedDynamicVariables.data() + range.offset + 3, 3);
        intWrenches(childLinkIndex) = iDynTree::Wrench(force, torque);


        // If the joint is not fixed, we can compute the joint torque
        joint->computeJointTorque(jointPos,
                                  intWrenches(childLinkIndex),
                                  parentLinkIndex,
                                  childLinkIndex,
                       ...
                       const CompareEstimatorsOptions& compareEstimatorsOptions)
{
    ...
    iDynTree::Triplets measurementCovarianceTriplets;
    ...
    measurementCovarianceTriplets.setDiagonalMatrix(linAccRange, linAccRange, lowCovariance);
    ...
    measurementCovarianceTriplets.setDiagonalMatrix(angAccRange, angAccRange, lowCovariance);
    for (int ftIdx = 0; ftIdx < usedSensors.getNrOfSensors(iDynTree::SIX_AXIS_FORCE_TORQUE); ftIdx++)
    {
        ...
        measurementCovarianceTriplets.setDiagonalMatrix(ftRange, ftRange, lowCovariance);
    }
    for (int dofIdx = 0; dofIdx < model.getNrOfDOFs(); dofIdx++)
    {
        ...
        measurementCovarianceTriplets.setDiagonalMatrix(jointAccRange, jointAccRange, lowCovariance);
    }
    ...
    for (iDynTree::LinkIndex linkIdx = 0; linkIdx < model.getNrOfLinks(); linkIdx++)
    {
        ...
        if (linkUnknown)
        {
            measurementCovarianceTriplets.setDiagonalMatrix(extWrenchRange, extWrenchRange, highCovariance);
        }
        else
        {
            measurementCovarianceTriplets.setDiagonalMatrix(extWrenchRange, extWrenchRange, lowCovariance);
        }
    ...
    iDynTree::SparseMatrix<iDynTree::ColumnMajor> measurementsCovariance(berdyHelper.getNrOfSensorsMeasurements(),
                                                                         berdyHelper.getNrOfSensorsMeasurements());
    measurementsCovariance.setFromTriplets(measurementCovarianceTriplets);
    probabilistEstimator.setMeasurementsPriorCovariance(measurementsCovariance);
    // std::cerr << "measurementsCovariance:\n" << toEigen(measurementsCovariance) << std::endl;

    tic = clock();
    probabilistEstimator.updateEstimateInformationFloatingBase(jointPos, jointVel, measures, imuFrameIdx, imuAngularVel);
    ASSERT_IS_TRUE(ok);
    ...
    extractJointTorquesAndContactForces(berdyHelper, jointPos, estimatedDynamicVariables,
