...
void diffArticulatedBodyAlgorithmInternalBuffers(ArticulatedBodyAlgorithmInternalBuffers& bufs,
                                                 ArticulatedBodyAlgorithmInternalBuffers& perturbed)
{
    std::cout << "diffArticulatedBodyAlgorithmInternalBuffers with " << bufs.linksVel.getNrOfLinks()
              << " links " << std::endl;
    for (LinkIndex link = 0; link < bufs.linksVel.getNrOfLinks(); link++)
    {
        ...
        std::cout << "linksBiasAcceleration diff \n "
                  << toEigen(bufs.linksBiasAcceleration(link))
                         - toEigen(perturbed.linksBiasAcceleration(link))
                  << std::endl;
        std::cout << "linksAccelerations diff \n"
                  << toEigen(bufs.linksAccelerations(link))
                         - toEigen(perturbed.linksAccelerations(link))
                  << std::endl;
        std::cout << "linkBiasWrench diff \n"
                  << toEigen(bufs.linksBiasWrench(link)) - toEigen(perturbed.linksBiasWrench(link))
                  << std::endl;
    }
}
                              ...
                              bool verbose = true)
{
    ...
    for (size_t l = 0; l < nrOfLinks; l++)
    {
        if (false)
        {
            ...
            std::cerr << "Norm of diff in dVb_linkBiasAcceleration : "
                      << (toEigen(bufs.dVb_linkBiasAcceleration[l])
                          - toEigen(numBufs.dVb_linkBiasAcceleration[l]))
                             .norm()
                      << std::endl;
            std::cerr << "Norm of diff in dVb_linksAccelerations : "
                      << (toEigen(bufs.dVb_linksAccelerations[l])
                          - toEigen(numBufs.dVb_linksAccelerations[l]))
                             .norm()
                      << std::endl;
            std::cerr << "Norm of diff in dVb_linkBiasWrench : "
                      << (toEigen(bufs.dVb_linkBiasWrench[l])
                          - toEigen(numBufs.dVb_linkBiasWrench[l]))
                             .norm()
                      << std::endl;
    ...
    {
        if (false)
            std::cerr << "~~~~~~~~~ Checking buffers for dofDeriv " << dofDeriv << std::endl;
        // Check link buffers
        for (size_t l = 0; l < nrOfLinks; l++)
        {
            if (false)
            {
                ...
                std::cerr << "Norm of diff in linksVel : "
                          << (toEigen(bufs.dVel[dofDeriv].linksVel(l))
                              - toEigen(numBufs.dVel[dofDeriv].linksVel(l)))
                                 .norm()
                          << std::endl;
                std::cerr << "Norm of diff in linksBiasAcceleration : "
                          << (toEigen(bufs.dVel[dofDeriv].linksBiasAcceleration(l))
                              - toEigen(numBufs.dVel[dofDeriv].linksBiasAcceleration(l)))
                                 .norm()
                          << std::endl;
                std::cerr << "Norm of diff in linksAccelerations : "
                          << (toEigen(bufs.dVel[dofDeriv].linksAccelerations(l))
                              - toEigen(numBufs.dVel[dofDeriv].linksAccelerations(l)))
                                 .norm()
                          << std::endl;
                std::cerr << "Norm of diff in linkBiasWrench : "
                          << (toEigen(bufs.dVel[dofDeriv].linksBiasWrench(l))
                              - toEigen(numBufs.dVel[dofDeriv].linksBiasWrench(l)))
                                 .norm()
                          << std::endl;
                // std::cerr << "Norm of diff in pa : "
                //       <<
                //       (toEigen(bufs.dVel[dofDeriv].pa(l))-toEigen(numBufs.dVel[dofDeriv].pa(l))).norm()
                //       << std::endl;
            }
        }
        ...
        for (size_t dof = 0; dof < nrOfDofs; dof++)
        {
            if (verbose)
            {
                ...
                std::cerr << "Norm of diff in u : "
                          << fabs(bufs.dVel[dofDeriv].u(dof) - numBufs.dVel[dofDeriv].u(dof))
                          << std::endl;
            }
        }
    }
    ...
    for (size_t dofDeriv = 0; dofDeriv < nrOfDofs; dofDeriv++)
    {
        ...
        for (size_t l = 0; l < nrOfLinks; l++)
        {
            if (true)
            {
                std::cerr << "Check difference in buffers for link " << l << std::endl;
                std::cerr << "Norm of diff in linksVel : "
                          << (toEigen(bufs.dPos[dofDeriv].linksVel(l))
                              - toEigen(numBufs.dPos[dofDeriv].linksVel(l)))
                                 .norm()
                          << std::endl;
                std::cerr << "Norm of diff in linksBiasAcceleration : "
                          << (toEigen(bufs.dPos[dofDeriv].linksBiasAcceleration(l))
                              - toEigen(numBufs.dPos[dofDeriv].linksBiasAcceleration(l)))
                                 .norm()
                          << std::endl;
                std::cerr << "Norm of diff in linksAccelerations : "
                          << (toEigen(bufs.dPos[dofDeriv].linksAccelerations(l))
                              - toEigen(numBufs.dPos[dofDeriv].linksAccelerations(l)))
                                 .norm()
                          << std::endl;
                std::cerr << "Norm of diff in linkBiasWrench : "
                          << (toEigen(bufs.dPos[dofDeriv].linksBiasWrench(l))
                              - toEigen(numBufs.dPos[dofDeriv].linksBiasWrench(l)))
                                 .norm()
                          << std::endl;
                // std::cerr << "Norm of diff in pa : "
                //       <<
        ...
        for (size_t dof = 0; dof < nrOfDofs; dof++)
        {
            if (true)
            {
                std::cerr << "Check difference in buffers for dof " << dof << std::endl;
                std::cerr << "Norm of diff in u : "
                          << fabs(bufs.dPos[dofDeriv].u(dof) - numBufs.dPos[dofDeriv].u(dof))
                          << std::endl;
                std::cerr << "Norm of diff in D : "
                          << fabs(bufs.dPos[dofDeriv].D(dof) - numBufs.dPos[dofDeriv].D(dof))
                          << std::endl;
                std::cerr << " D " << bufs.aba.D(dof) << std::endl;
            }
        }
    }
}
                                        ...
                                        size_t column)
{
    ...
    toEigen(A).block<6, 1>(6 + nrOfDofs, column)
        = (toEigen(perturbedAcc.baseAcc()) - toEigen(acc.baseAcc())) / step;

    for (size_t dof = 0; dof < acc.getNrOfDOFs(); dof++)
    {
        A(6 + nrOfDofs + 6 + dof, column)
            = (perturbedAcc.jointAcc()(dof) - acc.jointAcc()(dof)) / step;
    }
}
 ...
 *
 */
void fillLinBufsWithNumericalDerivativesWrtBaseTwist(
    const ArticulatedBodyAlgorithmInternalBuffers& bufsLower,
    ...
    ForwardDynamicsLinearizationInternalBuffers& numBufs)
{
    ...
    {
        toEigen(numBufs.dVb_linkBiasAcceleration[l]).block<6, 1>(0, h)
            = (toEigen(bufsUpper.linksBiasAcceleration(l))
               - toEigen(bufsLower.linksBiasAcceleration(l)))
              / step;
        toEigen(numBufs.dVb_linksAccelerations[l]).block<6, 1>(0, h)
            = (toEigen(bufsUpper.linksAccelerations(l)) - toEigen(bufsLower.linksAccelerations(l)))
              / step;
        toEigen(numBufs.dVb_linkBiasWrench[l]).block<6, 1>(0, h)
            = (toEigen(bufsUpper.linksBiasWrench(l)) - toEigen(bufsLower.linksBiasWrench(l)))
              / step;
    }
}
...
void fillLinBufsWithNumericalDerivativesWrtJointQuantity(
    const ArticulatedBodyAlgorithmInternalBuffers& bufsLower,
    const ArticulatedBodyAlgorithmInternalBuffers& bufsUpper,
    const double step,
    ArticulatedBodyAlgorithmInternalBuffers& numBufs)
{
    ...
    {
        setSpatialVectorFromEigen(numBufs.linksVel(l),
                                  (toEigen(bufsUpper.linksVel(l)) - toEigen(bufsLower.linksVel(l)))
                                      / step);
        setSpatialVectorFromEigen(numBufs.linksBiasAcceleration(l),
                                  (toEigen(bufsUpper.linksBiasAcceleration(l))
                                   - toEigen(bufsLower.linksBiasAcceleration(l)))
                                      / step);
        setSpatialVectorFromEigen(numBufs.linksAccelerations(l),
                                  (toEigen(bufsUpper.linksAccelerations(l))
                                   - toEigen(bufsLower.linksAccelerations(l)))
                                      / step);
        setSpatialVectorFromEigen(numBufs.linksBiasWrench(l),
                                  (toEigen(bufsUpper.linksBiasWrench(l))
                                   - toEigen(bufsLower.linksBiasWrench(l)))
                                      / step);
                                           ...
                                           FreeFloatingStateLinearization& A)
{
    ...
    for (size_t h = 0; h < model.getNrOfDOFs(); h++)
    {
        ...
        ArticulatedBodyAlgorithm(model,
                                 traversal,
                                 perturbedRobotPosLower,
                                 perturbedRobotVelLower,
                                 linkExtWrenches,
                                 jointTorques,
                                 perturbedBufsLower,
                                 perturbedAccLower);
    ...
    for (size_t h = 0; h < 6; h++)
    {
        ...
        fillLinBufsWithNumericalDerivativesWrtBaseTwist(perturbedBufsLower,
                                                        perturbedBufs,
                                                        h,
                                                        step,
                                                        bufsNum);
        ...
        // Fill the matrix column
        fillLowerColumnLinearizationMatrix(A,
                                           perturbedAccLower,
                                           perturbedAcc,
                                           step,
                                           h + 6 + model.getNrOfDOFs());
    ...
    for (size_t h = 0; h < model.getNrOfDOFs(); h++)
    {
        ...
        fillLowerColumnLinearizationMatrix(A,
                                           ...
                                           perturbedAcc,
                                           step,
                                           h + 6 + 6 + model.getNrOfDOFs());

        // we compute also the buffers values using the numerical differences
        fillLinBufsWithNumericalDerivativesWrtJointQuantity(perturbedBufsLower,
                                                            perturbedBufs,
                                                            step,
                                                            bufsNum.dVel[h]);
 ...
 */
void checkABAandABALinearizationAreConsistent(const Model& model, const Traversal& traversal)
{
    // Input data
    ...
    ForwardDynamicsLinearizationNumerical(model,
                                          traversal,
                                          robotPos,
                                          robotVel,
                                          linkExtWrenches,
    ...
    ForwardDynamicsLinearization(model,
                                 traversal,
                                 robotPos,
                                 robotVel,
                                 linkExtWrenches,
...
int main()
{
    ...
    {
        std::cerr << "Checking DynamicsLinearization test on random chain with " << joints
                  << " joints " << std::endl;
    ...
    for (unsigned int joints = 0; joints < 20; joints++)
    {
        std::cerr << "Checking DynamicsLinearization test on on random tree with " << joints
                  << "joints " << std::endl;
        Model model
            = getRandomModel(joints, 10, SIMPLE_JOINT_TYPES | JOINT_REVOLUTE_SO2 | JOINT_SPHERICAL);
        checkABAandABALinearizationAreConsistent(model);
    }
