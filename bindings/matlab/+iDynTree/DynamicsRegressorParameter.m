classdef DynamicsRegressorParameter < SwigRef
  methods
    ...
    function varargout = type(self, varargin)
      ...
      if nargin==1
        nargoutchk(0, 1)
        varargout{1} = iDynTreeMATLAB_wrap(785, self);
      else
        nargoutchk(0, 0)
        iDynTreeMATLAB_wrap(786, self, varargin{1});
      end
    end
    ...
    function self = DynamicsRegressorParameter(varargin)
      if nargin~=1 || ~ischar(varargin{1}) || ~strcmp(varargin{1},'_swigCreate')
        ...
        %self.swigInd = iDynTreeMATLAB_wrap(790, varargin{:});
        tmp = iDynTreeMATLAB_wrap(790, varargin{:}); % FIXME
