classdef DynamicsRegressorParametersList < SwigRef
  methods
    function varargout = parameters(self, varargin)
      ...
      if nargin==1
        nargoutchk(0, 1)
        varargout{1} = iDynTreeMATLAB_wrap(792, self);
      else
        nargoutchk(0, 0)
        iDynTreeMATLAB_wrap(793, self, varargin{1});
      end
    end
    ...
    function self = DynamicsRegressorParametersList(varargin)
      if nargin~=1 || ~ischar(varargin{1}) || ~strcmp(varargin{1},'_swigCreate')
        ...
        %self.swigInd = iDynTreeMATLAB_wrap(799, varargin{:});
        tmp = iDynTreeMATLAB_wrap(799, varargin{:}); % FIXME
