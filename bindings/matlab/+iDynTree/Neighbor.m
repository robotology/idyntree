classdef Neighbor < SwigRef
  methods
    function varargout = neighborLink(self, varargin)
      ...
      if nargin==1
        nargoutchk(0, 1)
        varargout{1} = iDynTreeMATLAB_wrap(661, self);
      else
        nargoutchk(0, 0)
        iDynTreeMATLAB_wrap(662, self, varargin{1});
      end
    end
    ...
    function self = Neighbor(varargin)
      if nargin~=1 || ~ischar(varargin{1}) || ~strcmp(varargin{1},'_swigCreate')
        ...
        %self.swigInd = iDynTreeMATLAB_wrap(665, varargin{:});
        tmp = iDynTreeMATLAB_wrap(665, varargin{:}); % FIXME
