classdef GeomVector3__AngularMotionVector3 < iDynTree.Vector3
  methods
    function varargout = semantics(self, varargin)
      ...
      if nargin==1
        nargoutchk(0, 1)
        varargout{1} = iDynTreeMATLAB_wrap(231, self);
      else
        nargoutchk(0, 0)
        iDynTreeMATLAB_wrap(232, self, varargin{1});
      end
    end
    function self = GeomVector3__AngularMotionVector3(varargin)
      ...
      if nargin~=1 || ~ischar(varargin{1}) || ~strcmp(varargin{1},'_swigCreate')
        ...
        %self.swigInd = iDynTreeMATLAB_wrap(233, varargin{:});
        tmp = iDynTreeMATLAB_wrap(233, varargin{:}); % FIXME
        self.swigInd = tmp.swigInd;
        tmp.swigInd = uint64(0);
      end
    end
    function varargout = setSemantics(self,varargin)
      [varargout{1:nargout}] = iDynTreeMATLAB_wrap(234, self, varargin{:});
    end
    function varargout = changeCoordFrame(self,varargin)
      [varargout{1:max(1,nargout)}] = iDynTreeMATLAB_wrap(235, self, varargin{:});
    end
  ...
  methods(Static)
    function varargout = compose(varargin)
      [varargout{1:max(1,nargout)}] = iDynTreeMATLAB_wrap(236, varargin{:});
    end
    function varargout = inverse(varargin)
      [varargout{1:max(1,nargout)}] = iDynTreeMATLAB_wrap(237, varargin{:});
    end
  end
end
